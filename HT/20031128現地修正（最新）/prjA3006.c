...
//				dspstring("IrDA data ok\r\n");
		...
		case 10:/*ダウンロード*/
			...
			if(nRet >= 0)
			{
				...
				waittm(750);					/* 750*20ms(15秒)一時停止 2003/09/22 A.Tamura */
			}
			else if(nRet == -2)
			{
				erbz1();					/*エラーブザー鳴動制御 200ms*3回*/
		...
		case 20:/*アップロード*/
			SelectAnothDisp(mnScr2, 1);		/*サブ画面*/
			dspone(0x0c);
			/* 2003/11/27 A.Tamura	ｱｯﾌﾟﾛｰﾄﾞ中にﾄﾗﾌﾞﾙがあるとこの画面でSTOPしてしまうので
						メッセージを変更しました。
			dspstring("指示ﾃﾞｰﾀをｱｯﾌﾟﾛｰﾄﾞし\r\n");
			dspstring("ています。\r\n");
			dspstring("暫くお待ち下さい。\r\n");
			*/
			dspstring("ﾃﾞｰﾀをｱｯﾌﾟﾛｰﾄﾞ中です｡");
			dspstring("この画面が5秒以上続く");
			dspstring("時は電源OFFして下さい");
			nRet = subUpload();
			if(nRet >= 0)
			{
				ctrlbuzz(0xff, 50, 0);		/*確認用ブザー鳴動制御 鳴動,50*20ms,通常*/
				...
				dspstring(asTmp);
				flshkb(0, asTmp);				/*キーボード入力バッファクリア*/
				waittm(250);					/* 250*20ms(5秒)一時停止 2003/09/22 A.Tamura */
			}
			else
			{
				erbz1();					/*エラーブザー鳴動制御 200ms*3回*/
