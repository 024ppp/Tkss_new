...
//  Unit3.cpp
//    【出荷検査システム－製品規格マスター】
//    素材検査項目フォーム（TForm3クラス）のメンバ関数の定義
//
//  作成者
//    田村　敦司
//
...
void __fastcall TForm3::FormActivate(TObject *Sender)
{
    ...
    for( i=1; i<=Form1->Lbx_DTKSHIN_SZ->Items->Count; i++ ) {
        sBuf = Form1->Lbx_DTKSHIN_SZ->Items->Strings[i-1];
		if( i == 1 ){
    ...
    // 利用しないタブを隠します
    for( i=Form1->Lbx_DTKSHIN_SZ->Items->Count + 1; i<=6; i++ ) {
    ...
    if( !Form1->Query1->Eof ){
        ...
        // メモリ確保数を取得
		iKSM13_DATA_SEL_Cnt = Form1->Query1->RecordCount * 20;
	...
	while( !Form1->Query1->Eof ){
       	...
       	if( VarIsNull(Form1->Query1->FieldValues["DTKSHIN"]) ){
			sDTKSHIN = "";
		} else{
			sDTKSHIN = Form1->Query1->FieldValues["DTKSHIN"];			// 代表得意先品番
		}

        // リード線長さ
		sBuf = Form1->Query1->FieldValues["SK_LD_LNG"];
        ...
        // ブラシ幅（コンミ：内径）
		sBuf = Form1->Query1->FieldValues["SK_BURASI"];
        if( sBuf.ToIntDef(0) > 0 ){
			strcpy( pKSM13_DATA_SEL[i].DTKSHIN		,sDTKSHIN.c_str() );// 代表得意先品番
            if( AnsiString(Form1->Query1->FieldValues["KEIJKBN"]) != "5" ){
            	sBuf = "2";
				strcpy( pKSM13_DATA_SEL[i].KOMK_NO		,sBuf.c_str() );	// 項目NO
            	sBuf = "12";
				strcpy( pKSM13_DATA_SEL[i].ORDER_NO		,sBuf.c_str() );	// 表示順
        ...
        // ブラシ幅縦押し（コンミ：外径）
		sBuf = Form1->Query1->FieldValues["SK_BURASIT"];
        if( sBuf.ToIntDef(0) > 0 ){
			strcpy( pKSM13_DATA_SEL[i].DTKSHIN		,sDTKSHIN.c_str() );// 代表得意先品番
            if( AnsiString(Form1->Query1->FieldValues["KEIJKBN"]) != "5" ){
            	sBuf = "4";
        ...
        // 見掛比重
		sBuf = Form1->Query1->FieldValues["SK_BOTAI"];
        if( sBuf.ToIntDef(0) > 0 ){
			strcpy( pKSM13_DATA_SEL[i].DTKSHIN		,sDTKSHIN.c_str() );// 代表得意先品番
            sBuf = "6";
			strcpy( pKSM13_DATA_SEL[i].KOMK_NO		,sBuf.c_str() );	// 項目NO
        ...
        // リード線取付強度
		sBuf = Form1->Query1->FieldValues["SK_LD_HP"];
        if( sBuf.ToIntDef(0) > 0 ){
			strcpy( pKSM13_DATA_SEL[i].DTKSHIN		,sDTKSHIN.c_str() );// 代表得意先品番
            sBuf = "7";
			strcpy( pKSM13_DATA_SEL[i].KOMK_NO		,sBuf.c_str() );	// 項目NO
            ...
            sBuf = "1";
			strcpy( pKSM13_DATA_SEL[i].KIKA_DECI	,sBuf.c_str() );	// 規格小数点以下桁数
			sBuf = Form1->Query1->FieldValues["LD_HP_KYO"];
			strcpy( pKSM13_DATA_SEL[i].KIKA_NUM1	,sBuf.c_str() );	// 規格値１
			sBuf = "0";
			strcpy( pKSM13_DATA_SEL[i].KIKA_NUM2	,sBuf.c_str() );	// 規格値２
			sBuf = "0";
        ...
        // リード線取付抵抗
		sBuf = Form1->Query1->FieldValues["SK_LD_TR"];
        if( sBuf.ToIntDef(0) > 0 ){
			strcpy( pKSM13_DATA_SEL[i].DTKSHIN		,sDTKSHIN.c_str() );// 代表得意先品番
            sBuf = "8";
			strcpy( pKSM13_DATA_SEL[i].KOMK_NO		,sBuf.c_str() );	// 項目NO
            ...
            sBuf = "リード線取付抵抗測定機";
			strcpy( pKSM13_DATA_SEL[i].KIKI_NM		,sBuf.c_str() );	// 測定機器名称
            sBuf = "mV/" + Form1->Query1->FieldValues["LD_TR_DENR"] + "A";
			strcpy( pKSM13_DATA_SEL[i].TANI			,sBuf.c_str() );	// 単位
            sBuf = "1";
			strcpy( pKSM13_DATA_SEL[i].KIKA_TYPE	,sBuf.c_str() );	// 規格タイプ
            sBuf = "2";
			...
			sBuf = "0";
			strcpy( pKSM13_DATA_SEL[i].KIKA_NUM1	,sBuf.c_str() );	// 規格値１
			sBuf = Form1->Query1->FieldValues["LD_TR_DENA"];
			strcpy( pKSM13_DATA_SEL[i].KIKA_NUM2	,sBuf.c_str() );	// 規格値２
			sBuf = "0";
			strcpy( pKSM13_DATA_SEL[i].KIKA_NUM3	,sBuf.c_str() );	// 規格値３
			sBuf = "";
        ...
        // 抵抗率
		sBuf = Form1->Query1->FieldValues["SK_KOYU"];
        if( sBuf.ToIntDef(0) > 0 ){
			strcpy( pKSM13_DATA_SEL[i].DTKSHIN		,sDTKSHIN.c_str() );// 代表得意先品番
            sBuf = "9";
			strcpy( pKSM13_DATA_SEL[i].KOMK_NO		,sBuf.c_str() );	// 項目NO
        ...
        // 曲げ強さ
		sBuf = Form1->Query1->FieldValues["SK_MAGE"];
        if( sBuf.ToIntDef(0) > 0 ){
			strcpy( pKSM13_DATA_SEL[i].DTKSHIN		,sDTKSHIN.c_str() );// 代表得意先品番
            sBuf = "10";
			strcpy( pKSM13_DATA_SEL[i].KOMK_NO		,sBuf.c_str() );	// 項目NO
        ...
        // 曲げ強さ（現物）
		sBuf = Form1->Query1->FieldValues["SK_MAGEG"];
        if( sBuf.ToIntDef(0) > 0 ){
			strcpy( pKSM13_DATA_SEL[i].DTKSHIN		,sDTKSHIN.c_str() );// 代表得意先品番
            sBuf = "11";
			strcpy( pKSM13_DATA_SEL[i].KOMK_NO		,sBuf.c_str() );	// 項目NO
        ...
        // 硬度ＨｓＣ（圧面）
		sBuf = Form1->Query1->FieldValues["SK_HSC_A"];
        if( sBuf.ToIntDef(0) > 0 ){
			strcpy( pKSM13_DATA_SEL[i].DTKSHIN		,sDTKSHIN.c_str() );// 代表得意先品番
            sBuf = "12";
			strcpy( pKSM13_DATA_SEL[i].KOMK_NO		,sBuf.c_str() );	// 項目NO
        ...
        // 硬度ＨｓＣ（側面）
		sBuf = Form1->Query1->FieldValues["SK_HSC_S"];
        if( sBuf.ToIntDef(0) > 0 ){
			strcpy( pKSM13_DATA_SEL[i].DTKSHIN		,sDTKSHIN.c_str() );// 代表得意先品番
            sBuf = "13";
			strcpy( pKSM13_DATA_SEL[i].KOMK_NO		,sBuf.c_str() );	// 項目NO
        ...
        // 硬度ＨｓＤ（圧面）
		sBuf = Form1->Query1->FieldValues["SK_HSD_A"];
        if( sBuf.ToIntDef(0) > 0 ){
			strcpy( pKSM13_DATA_SEL[i].DTKSHIN		,sDTKSHIN.c_str() );// 代表得意先品番
            sBuf = "14";
			strcpy( pKSM13_DATA_SEL[i].KOMK_NO		,sBuf.c_str() );	// 項目NO
        ...
        // 硬度ＨｓＤ（側面）
		sBuf = Form1->Query1->FieldValues["SK_HSD_S"];
        if( sBuf.ToIntDef(0) > 0 ){
			strcpy( pKSM13_DATA_SEL[i].DTKSHIN		,sDTKSHIN.c_str() );// 代表得意先品番
            sBuf = "15";
			strcpy( pKSM13_DATA_SEL[i].KOMK_NO		,sBuf.c_str() );	// 項目NO
        ...
        // 座屈
		sBuf = Form1->Query1->FieldValues["SK_ZAKUT"];
        if( sBuf.ToIntDef(0) > 0 ){
			strcpy( pKSM13_DATA_SEL[i].DTKSHIN		,sDTKSHIN.c_str() );// 代表得意先品番
            sBuf = "16";
			strcpy( pKSM13_DATA_SEL[i].KOMK_NO		,sBuf.c_str() );	// 項目NO
        ...
        if( sBuf.ToIntDef(0) > 0 ){
            ...
            sBuf = "2";
			strcpy( pKSM13_DATA_SEL[i].KIKA_DECI	,sBuf.c_str() );	// 規格小数点以下桁数
			sBuf = Form1->Query1->FieldValues["HIST_MIN"];
			strcpy( pKSM13_DATA_SEL[i].KIKA_NUM1	,sBuf.c_str() );	// 規格値１
			sBuf = Form1->Query1->FieldValues["HIST_MAX"];
			strcpy( pKSM13_DATA_SEL[i].KIKA_NUM2	,sBuf.c_str() );	// 規格値２
			sBuf = "0";
			strcpy( pKSM13_DATA_SEL[i].KIKA_NUM3	,sBuf.c_str() );	// 規格値３
			sBuf = "";
    ...
    //１行目を自動選択します
    SGr_ListSelectCell( SGr_List1, 1, 1, true);
    ...
    if( !Form1->Query1->Eof ){
        ...
        // メモリ確保数を取得
		iKSM13_DATA_ALL_Cnt = Form1->Query1->RecordCount;
	...
	while( !Form1->Query1->Eof ){
		...
		sBuf = Form1->Query1->FieldValues["KOMK_NO"];
		strcpy( pKSM13_DATA_ALL[i].KOMK_NO		,sBuf.c_str() );	// 項目NO

		sBuf = Form1->Query1->FieldValues["ORDER_NO"];
		strcpy( pKSM13_DATA_ALL[i].ORDER_NO		,sBuf.c_str() );	// 表示順

        if( VarIsNull(Form1->Query1->FieldValues["KOMK_NM"]) ){
			sBuf = "";
		} else{
			sBuf = Form1->Query1->FieldValues["KOMK_NM"];
		}
		strcpy( pKSM13_DATA_ALL[i].KOMK_NM		,sBuf.c_str() );	// 測定項目（日本語）

        if( VarIsNull(Form1->Query1->FieldValues["KOMK_ENG"]) ){
			sBuf = "";
		} else{
			sBuf = Form1->Query1->FieldValues["KOMK_ENG"];
        }
		strcpy( pKSM13_DATA_ALL[i].KOMK_ENG		,sBuf.c_str() );	// 測定項目（英語）

		sBuf = Form1->Query1->FieldValues["KIKI_NO"];
		strcpy( pKSM13_DATA_ALL[i].KIKI_NO		,sBuf.c_str() );	// 測定機器NO

        if( VarIsNull(Form1->Query1->FieldValues["KIKI_NM"]) ){
			sBuf = "";
		} else{
			sBuf = Form1->Query1->FieldValues["KIKI_NM"];
        }
		strcpy( pKSM13_DATA_ALL[i].KIKI_NM		,sBuf.c_str() );	// 測定機器名称

        if( VarIsNull(Form1->Query1->FieldValues["TANI"]) ){
			sBuf = "";
		} else{
...
//
//---------------------------------------------------------------------------
void __fastcall TForm3::BtnF08F09Click(TObject *Sender)
{
    ...
    if( iRowSelTop != iRowSelBottom ){
    	...
    	for( i=iRowSelTop; i<=iRowSelBottom; i++ ) {
            ...
            if( bt->Name == "BtnF08" ){
                // 「Ｆ８：追加」
                Form1->Ins_SGr_SOKUT(Form1->SGr_SOKUT->RowCount-1);
            }
            else {
                // 「Ｆ９：挿入」
                Form1->Ins_SGr_SOKUT(Form1->SGr_SOKUT->Row);
            	if( i != iRowSelBottom )	Form1->SGr_SOKUT->Row = Form1->SGr_SOKUT->Row + 1;
            }
    	}
...
//
//---------------------------------------------------------------------------
void __fastcall TForm3::Pnl_ListResize(TObject *Sender)
{
    int iBMarg;
...
//
//---------------------------------------------------------------------------
void __fastcall TForm3::Rdo_KIKAKU_SHITEI_HANIClick(TObject *Sender)
{
...
//
//---------------------------------------------------------------------------
void __fastcall TForm3::Rdo_KIKAKU_SHITEI_KIJYUNClick(TObject *Sender)
{
...
//
//---------------------------------------------------------------------------
void __fastcall TForm3::Rdo_KIKAKU_SHITEI_MOJIClick(TObject *Sender)
{
    Pnl_KIKAKU_HANI->Visible   = false;
    Pnl_KIKAKU_KIJYUN->Visible = false;
...
//
//---------------------------------------------------------------------------
void __fastcall TForm3::Rdo_KIKAKU_SYURUI_MINMAXClick(TObject *Sender)
{
...
//
//---------------------------------------------------------------------------
void __fastcall TForm3::Rdo_KIKAKU_SYURUI_MAXClick(TObject *Sender)
{
...
//
//---------------------------------------------------------------------------
void __fastcall TForm3::Rdo_KIKAKU_SYURUI_MINClick(TObject *Sender)
{
    Nmb_KIKAKU_HANI_MIN->Enabled	= true;
...
//
//---------------------------------------------------------------------------
void __fastcall TForm3::PgCtr_ListChange(TObject *Sender)
{
    ...
    if( PgCtr_List->ActivePage->Name == "TabSheet1" ){
    	...
    	SGr_ListSelectCell( SGr_List1, 1, SGr_List1->Row, true);
	    SGr_ListClick( SGr_List1 );
    	...
    	SGr_ListSelectCell( SGr_List3, 1, SGr_List3->Row, true);
    	...
    	SGr_ListSelectCell( SGr_List4, 1, SGr_List4->Row, true);
    	...
    	SGr_ListSelectCell( SGr_List5, 1, SGr_List5->Row, true);
    	...
    	SGr_ListSelectCell( SGr_List6, 1, SGr_List6->Row, true);
...
//
//---------------------------------------------------------------------------
void __fastcall TForm3::Rdo_VALUE_NORMALClick(TObject *Sender)
{
...
//
//---------------------------------------------------------------------------
void __fastcall TForm3::Rdo_VALUE_KAKUDOClick(TObject *Sender)
{
    Nmb_VALUE_DOUBLE->Enabled     = true;
    Nmb_VALUE_DOUBLE->Font->Color = clBlack;
...
//
//---------------------------------------------------------------------------
void __fastcall TForm3::Rdo_VALUE_OKClick(TObject *Sender)
{
...
//
//---------------------------------------------------------------------------
void __fastcall TForm3::Rdo_VALUE_TEXTClick(TObject *Sender)
{
    Nmb_VALUE_DOUBLE->Enabled     = false;
    Nmb_VALUE_DOUBLE->Font->Color = clGray;
...
//
//---------------------------------------------------------------------------
void __fastcall TForm3::DispSGr_List(TTabSheet *ts, int	type)
{
...
//
//---------------------------------------------------------------------------
void __fastcall TForm3::SGr_ListClick(TObject *Sender)
{
...
//
//---------------------------------------------------------------------------
void __fastcall TForm3::SGr_ListDrawCell(TObject *Sender, int ACol,
   ...
   if( ARow > 0 && ACol == 4 ){
        ...
        switch( sg->Cells[11][ARow].ToIntDef(0) ){
            ...
            case 2: // 基準値指定
        			...
        			// フォント（小）の設定
        			Form1->OrgFont = SelectObject( sg->Canvas->Handle, Form1->MyFont );
        			...
        			// 元のフォントに戻します
        			SelectObject( sg->Canvas->Handle, Form1->OrgFont );
...
//
//---------------------------------------------------------------------------
void __fastcall TForm3::SGr_ListSelectCell(TObject *Sender, int ACol,
...
//
//---------------------------------------------------------------------------
void __fastcall TForm3::Rdo_SOZAI_Click(TObject *Sender)
{
    ...
    if( PgCtr_List->ActivePage->Name == "TabSheet1" ){
    	...
    	SGr_ListSelectCell( SGr_List2, 1, 1, true);
	    SGr_ListClick( SGr_List2 );
    	...
    	SGr_ListSelectCell( SGr_List3, 1, 1, true);
	    SGr_ListClick( SGr_List3 );
    	...
    	SGr_ListSelectCell( SGr_List4, 1, 1, true);
	    SGr_ListClick( SGr_List4 );
    	...
    	SGr_ListSelectCell( SGr_List5, 1, 1, true);
	    SGr_ListClick( SGr_List5 );
    	...
    	SGr_ListSelectCell( SGr_List6, 1, 1, true);
	    SGr_ListClick( SGr_List6 );
...
//
//---------------------------------------------------------------------------
void __fastcall TForm3::Nmb_KIKAKU_DOUBLEExit(TObject *Sender)
{
...
//
//---------------------------------------------------------------------------
void __fastcall TForm3::SetSubName( AnsiString sHINBAN )
{
    AnsiString	sBuf;
    int			i;
