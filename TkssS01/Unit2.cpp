...
//---------------------------------------------------------------------------
//自動整理登録
int __fastcall TSparkyThread::AutoEntry()
{
	int iRtn;
	AnsiString sRtn;
	int i;
	...
	for ( i = 0; i < isDTKSHINNum; i++ ) {
		sRtn = UpdDataKSD21_UPDCNT(sDTKSHIN->Strings[i],sHINBAN->Strings[i],sLOTNO->Strings[i]);
		WriteErrLog(sRtn,"UpdDataKSD21_UPDCNT",KSD21.DTKSHIN,KSD21.HINBAN,KSD21.LOTNO);
	}
...
//---------------------------------------------------------------------------
//素材測定実績明細データ取得(KDX1)
int __fastcall TSparkyThread::GetKD_1(int iNo)
{
	AnsiString sBuf;
	AnsiString sErr;
	int iRtn;
	...
	//KDX1からデータを取得し、ServiceTkssS01->Q_KD_1にセット
	iRtn = SrchKD_1(iNo);
	//検索結果０件の場合は終了
	if ( iRtn < 0 ) {
		return(-1);
	...
	//ServiceTkssS01->Q_KD_1を1件ずつ見ていき、KDS21・KDS22・KDS23を検索
	ServiceTkssS01->Q_KD_1->First();
	while( !ServiceTkssS01->Q_KD_1->Eof ) {
		//ブラシ幅・内径・ブラシ縦・外径の場合
		//形状区分により、処理を続行するかを決定
		iKEIJYO_KBN_OK = 0;
		switch (iNo)
		...
		if ( iKEIJYO_KBN_OK == 0 ) {
			...
			if ( KSD22.InsFlag >= 0 ) {
				sSOKUTEI_SU2 = Null_S(ServiceTkssS01->Q_KSD22->FieldValues["SOKU_VAL_SU"]);
			...
			if ( KSD22.InsFlag >= 0 ) {
				sErr = UpdDataKSD22();
				WriteErrLog(sErr,"UpdDataKSD22",KSD21.DTKSHIN,KSD21.HINBAN,KSD21.LOTNO);
			} else {
				sErr = AddDataKSD22();
				WriteErrLog(sErr,"AddDataKSD22",KSD21.DTKSHIN,KSD21.HINBAN,KSD21.LOTNO);
			}
			...
			if ( KSD23.InsFlag >= 0 ) {
				sErr = DelDataKSD23();
				WriteErrLog(sErr,"DelDataKSD23",KSD21.DTKSHIN,KSD21.HINBAN,KSD21.LOTNO);
			}
			iSOKU_VAL_SU = atoi(KSD22.SOKU_VAL_SU.c_str());
			if ( iSOKU_VAL_SU > 10 ) {
				for ( i = 10; i < iSOKU_VAL_SU; i++ ) {
					sErr = AddDataKSD23(i);
					WriteErrLog(sErr,"AddDataKSD23(" + IntToStr(i) + ")",KSD21.DTKSHIN,KSD21.HINBAN,KSD21.LOTNO);
				}
			}
			...
			if ( KSD21.InsFlag >= 0 ) {
				sErr = UpdDataKSD21();
				WriteErrLog(sErr,"UpdDataKSD21",KSD21.DTKSHIN,KSD21.HINBAN,KSD21.LOTNO);
			} else {
				sErr = AddDataKSD21();
				WriteErrLog(sErr,"AddDataKSD21",KSD21.DTKSHIN,KSD21.HINBAN,KSD21.LOTNO);
			}
			...
			for ( i = 0; i < isDTKSHINNum; i++ ) {
				if ( KSD21.DTKSHIN == sDTKSHIN->Strings[i] &&
					 ...
					 KSD21.HINBAN  == sHINBAN->Strings[i] ) {
					iFlag = 1;
					break;
				}
			}
			if ( iFlag == 0 ) {
				...
				sHINBAN->Add(KSD21.HINBAN);
				isDTKSHINNum++;
			}
		}

		ServiceTkssS01->Q_KD_1->Next();
	}
...
//---------------------------------------------------------------------------
//測定値最大・最小・判定を判断
void __fastcall TSparkyThread::CalcKD_1(int iNo)
{
	int i;
	int iSOKUTEI_SU;
	float fSOKUTEI;
...
//---------------------------------------------------------------------------
//KDX1からデータを取得し、ServiceTkssS01->Q_KD_1にセット
int __fastcall TSparkyThread::SrchKD_1(int iNo)
{
	AnsiString sBuf;

	//Initial Query
	ServiceTkssS01->Q_KD_1->Close();
	ServiceTkssS01->Q_KD_1->SQL->Clear();
	...
	ServiceTkssS01->Q_KD_1->SQL->Add(sBuf);
	ServiceTkssS01->Q_KD_1->Open();
	ServiceTkssS01->Q_KD_1->Active = true;

	if ( ServiceTkssS01->Q_KD_1->Bof && ServiceTkssS01->Q_KD_1->Eof ) {
		//検索結果 ０件
		return(-1);
	}
...
//---------------------------------------------------------------------------
//素材測定実績明細データ構造体にセット(KDX1)
void __fastcall TSparkyThread::SetKD_1(int iNo)
{

	//ServiceTkssS01->Q_KD_1を構造体にセット
	KD_1.KENSA_YMD  = Null_S(ServiceTkssS01->Q_KD_1->FieldValues["KENSA_YMD"]);
	KD_1.TOLEY_NO   = Null_S(ServiceTkssS01->Q_KD_1->FieldValues["TOLEY_NO"]);
	KD_1.SAMPLE_P   = Null_S(ServiceTkssS01->Q_KD_1->FieldValues["SAMPLE_P"]);
	KD_1.SOKUTEI_SU = Null_S(ServiceTkssS01->Q_KD_1->FieldValues["SOKUTEI_SU"]);
	KD_1.DTKSHIN    = Null_S(ServiceTkssS01->Q_KD_1->FieldValues["DTKSHIN"]);
	KD_1.LOTNO      = Null_S(ServiceTkssS01->Q_KD_1->FieldValues["LOTNO"]);
	KD_1.HINBAN 	= Null_S(ServiceTkssS01->Q_KD_1->FieldValues["HINBAN"]);
	KD_1.ZISCOD 	= Null_S(ServiceTkssS01->Q_KD_1->FieldValues["ZISCOD"]);
	KD_1.ZISNAM 	= Null_S(ServiceTkssS01->Q_KD_1->FieldValues["ZISNAM"]);
	KD_1.KIKA_NUM1 	= Null_S(ServiceTkssS01->Q_KD_1->FieldValues["KIKA_NUM1"]);
	KD_1.KIKA_NUM2 	= Null_S(ServiceTkssS01->Q_KD_1->FieldValues["KIKA_NUM2"]);
	KD_1.KIKA_SUB1 	= Null_S(ServiceTkssS01->Q_KD_1->FieldValues["KIKA_SUB1"]);
	KD_1.KIKA_SUB2 	= Null_S(ServiceTkssS01->Q_KD_1->FieldValues["KIKA_SUB2"]);
	KD_1.KEIJYO_KBN = Null_S(ServiceTkssS01->Q_KD_1->FieldValues["KEIJYO_KBN"]);
	KD_1.SOKUTEI[0] = Null_S(ServiceTkssS01->Q_KD_1->FieldValues["SOKUTEI_1"]);
	KD_1.SOKUTEI[1] = Null_S(ServiceTkssS01->Q_KD_1->FieldValues["SOKUTEI_2"]);
	KD_1.SOKUTEI_SU = Null_S(ServiceTkssS01->Q_KD_1->FieldValues["SOKUTEI_SU"]);
	KD_1.SOKUTEI_NM = Null_S(ServiceTkssS01->Q_KD_1->FieldValues["SOKUTEI_NM"]);
	KD_1.SOKUTE_AVE = Null_S(ServiceTkssS01->Q_KD_1->FieldValues["SOKUTE_AVE"]);
	KD_1.ADDYMD 	= Null_S(ServiceTkssS01->Q_KD_1->FieldValues["ADDYMD"]);
	KD_1.ADDTIM 	= Null_S(ServiceTkssS01->Q_KD_1->FieldValues["ADDTIM"]);
	KD_1.UPDYMD 	= Null_S(ServiceTkssS01->Q_KD_1->FieldValues["UPDYMD"]);
	KD_1.UPDTIM 	= Null_S(ServiceTkssS01->Q_KD_1->FieldValues["UPDTIM"]);
	KD_1.UPDCHR 	= Null_S(ServiceTkssS01->Q_KD_1->FieldValues["UPDCHR"]);
	KD_1.UPDCNT 	= Null_S(ServiceTkssS01->Q_KD_1->FieldValues["UPDCNT"]);
	KD_1.MEMO		= Null_S(ServiceTkssS01->Q_KD_1->FieldValues["MEMO"]);
...
//---------------------------------------------------------------------------
//KDX2からデータを取得し、ServiceTkssS01->Q_KD_2にセット
int __fastcall TSparkyThread::SrchKD_2(int iNo)
{
	AnsiString sBuf;

	//Initial Query
	ServiceTkssS01->Q_KD_2->Close();
	ServiceTkssS01->Q_KD_2->SQL->Clear();
	...
	ServiceTkssS01->Q_KD_2->SQL->Add(sBuf);
	ServiceTkssS01->Q_KD_2->Open();
	ServiceTkssS01->Q_KD_2->Active = true;

	if ( ServiceTkssS01->Q_KD_2->Bof && ServiceTkssS01->Q_KD_2->Eof ) {
		//検索結果 ０件
		return(-1);
	}
...
//---------------------------------------------------------------------------
//素材測定実績明細データ構造体にセット(KDX2)
void __fastcall TSparkyThread::SetKD_2(int iNo)
{
	int i;

	//ServiceTkssS01->Q_KD_2を構造体にセット
	i = 2;
	ServiceTkssS01->Q_KD_2->First();
	while( !ServiceTkssS01->Q_KD_2->Eof ) {
		KD_1.SOKUTEI[i] = Null_S(ServiceTkssS01->Q_KD_2->FieldValues["SOKUTEICHI"]);
		i++;
		ServiceTkssS01->Q_KD_2->Next();
	}


}
//---------------------------------------------------------------------------
//KSD21からデータを取得し、ServiceTkssS01->Q_KSD21にセット
int __fastcall TSparkyThread::SrchKSD21(int iNo)
{
	AnsiString sBuf;

	//Initial Query
	ServiceTkssS01->Q_KSD21->Close();
	ServiceTkssS01->Q_KSD21->SQL->Clear();
	...
	ServiceTkssS01->Q_KSD21->SQL->Add(sBuf);
	ServiceTkssS01->Q_KSD21->Open();
	ServiceTkssS01->Q_KSD21->Active = true;

	if ( ServiceTkssS01->Q_KSD21->Bof && ServiceTkssS01->Q_KSD21->Eof ) {
		//検索結果 ０件
		return(-1);
	}
...
//---------------------------------------------------------------------------
//素材測定実績明細データ構造体にセット(KSD21)
void __fastcall TSparkyThread::SetKSD21(int iNo)
{
	int iRtn;
	int iBuf;
	int iUPDCHR1;
	...
	if ( KSD21.InsFlag == 0 ) {
		KSD21.DTKSCOD = Null_S(ServiceTkssS01->Q_KSD21->FieldValues["DTKSCOD"]);
		KSD21.DTKSNAM = Null_S(ServiceTkssS01->Q_KSD21->FieldValues["DTKSNAM"]);
		//KSD21.SOKUTEI_NUM = Null_S(ServiceTkssS01->Q_KSD21->FieldValues["SOKUTEI_NUM"]);
		//KSD21.SOZAI_OK = Null_S(ServiceTkssS01->Q_KSD21->FieldValues["SOZAI_OK"]);
		KSD21.MEMO = Null_S(ServiceTkssS01->Q_KSD21->FieldValues["MEMO"]);
		KSD21.SKTYMD = Null_S(ServiceTkssS01->Q_KSD21->FieldValues["SKTYMD"]);
		//KSD21.ADDYMD = Null_S(ServiceTkssS01->Q_KSD21->FieldValues["ADDYMD"]);
		//KSD21.ADDTIM = Null_S(ServiceTkssS01->Q_KSD21->FieldValues["ADDTIM"]);
		//KSD21.UPDYMD = FormatDateTime("yyyymmdd",Now());// 更新日
		//KSD21.UPDTIM = FormatDateTime("hhnn",Now());	// 更新時間

		KSD21.SKTYMD = Null_S(ServiceTkssS01->Q_KSD21->FieldValues["SKTYMD"]);	//測定日
		KSD21.UPDCHR = Null_S(ServiceTkssS01->Q_KSD21->FieldValues["UPDCHR"]);	//更新者
...
//---------------------------------------------------------------------------
//KSD22からデータを取得し、ServiceTkssS01->Q_KSD22にセット
int __fastcall TSparkyThread::SrchKSD22(int iNo)
{
	AnsiString sBuf;

	//Initial Query
	ServiceTkssS01->Q_KSD22->Close();
	ServiceTkssS01->Q_KSD22->SQL->Clear();
	...
	ServiceTkssS01->Q_KSD22->SQL->Add(sBuf);
	ServiceTkssS01->Q_KSD22->Open();
	ServiceTkssS01->Q_KSD22->Active = true;

	if ( ServiceTkssS01->Q_KSD22->Bof && ServiceTkssS01->Q_KSD22->Eof ) {
		//検索結果 ０件
		return(-1);
	}
...
//---------------------------------------------------------------------------
//素材測定実績明細データ構造体にセット(KSD22)
void __fastcall TSparkyThread::SetKSD22(int iNo)
{
	AnsiString sBuf;
	int i;
	int iSOKU_VAL_SU;
	...
	//実績明細データ（KSD22）より
	if ( KSD22.InsFlag == 0 ) {
		KSD22.ADDYMD = Null_S(ServiceTkssS01->Q_KSD22->FieldValues["ADDYMD"]);
		KSD22.ADDTIM = Null_S(ServiceTkssS01->Q_KSD22->FieldValues["ADDTIM"]);
		KSD22.UPDYMD = FormatDateTime("yyyymmdd",Now());// 更新日
		KSD22.UPDTIM = FormatDateTime("hhnn",Now());	// 更新時間
		KSD22.UPDCNT = Null_S(ServiceTkssS01->Q_KSD22->FieldValues["UPDCNT"]);
...
//---------------------------------------------------------------------------
//KSD23からデータを取得し、ServiceTkssS01->Q_KSD23にセット
int __fastcall TSparkyThread::SrchKSD23(int iNo)
{
	AnsiString sBuf;

	//Initial Query
	ServiceTkssS01->Q_KSD23->Close();
	ServiceTkssS01->Q_KSD23->SQL->Clear();

	// 問合せ実行
	sBuf = "";
	//SELECT文
	sBuf += " SELECT ";
	sBuf += " DTKSHIN, LOTNO, UNQ_NO, SOKU_NO, SOKU_VAL ";
	//テーブル名
	sBuf += " FROM KSD23 ";
	//WHERE文
	sBuf += " WHERE  ";
	...
	ServiceTkssS01->Q_KSD23->SQL->Add(sBuf);
	ServiceTkssS01->Q_KSD23->Open();
	ServiceTkssS01->Q_KSD23->Active = true;

	if ( ServiceTkssS01->Q_KSD23->Bof && ServiceTkssS01->Q_KSD23->Eof ) {
		//検索結果 ０件
		return(-1);
	}
...
//---------------------------------------------------------------------------
//KSD21初期化
void __fastcall TSparkyThread::FormatKSD21(int iNo)
{
...
//---------------------------------------------------------------------------
//KSD22初期化
void __fastcall TSparkyThread::FormatKSD22(int iNo)
{
	int i;
...
//---------------------------------------------------------------------------
//KSD23初期化
void __fastcall TSparkyThread::FormatKSD23(int iNo)
{
	KSD23.InsFlag = -1; 					// 新規の場合：-1
}
//---------------------------------------------------------------------------
//FixStr初期化
void __fastcall TSparkyThread::FormatFixStr(void)
{
...
//---------------------------------------------------------------------------
//製品規格マスタより得意先コード・得意先名称取得
int __fastcall TSparkyThread::GetDTKS(void)
{
	AnsiString sBuf;
	...
	ServiceTkssS01->Query1->First();
	KSD21.DTKSCOD = Null_S(ServiceTkssS01->Query1->FieldValues["DTKSCOD"]);
	KSD21.DTKSNAM = Null_S(ServiceTkssS01->Query1->FieldValues["DTKSNAM"]);
...
//---------------------------------------------------------------------------
//製品規格マスタより、形状区分取得
AnsiString __fastcall TSparkyThread::GetKEIJYO_KBN(void)
{
	AnsiString sBuf;
	AnsiString sKEIJYO_KBN;
	AnsiString sDTKSHIN;
	...
	sKEIJYO_KBN = "";
	sDTKSHIN    = Null_S(ServiceTkssS01->Q_KD_1->FieldValues["DTKSHIN"]);
	...
	ServiceTkssS01->Query1->First();
	sKEIJYO_KBN = Null_S(ServiceTkssS01->Query1->FieldValues["KEIJKBN"]);
...
//---------------------------------------------------------------------------
//Nullならスペースを返す
AnsiString __fastcall TSparkyThread::Null_S(Variant &vBuf)
{
	AnsiString sBuf;
...
//---------------------------------------------------------------------------
//備考strMにstrAを挿入
AnsiString __fastcall TSparkyThread::InsMEMO(AnsiString strM, int iNo, AnsiString strA)
{
	int iPos;
	int iPos2;
	int iPos3;
...
//---------------------------------------------------------------------------
//ログを出力
void __fastcall TSparkyThread::WriteErrLog(AnsiString sErrLog,  AnsiString sePrg,
								 	AnsiString seDTKSHIN,AnsiString seHINBAN,AnsiString seLOTNO)
{
	AnsiString sBuf;
	...
	sBuf += " HINBAN：" + seHINBAN  + AnsiString::StringOfChar(' ', 20-seHINBAN.Length());
	sBuf += " LOTNO：" + seLOTNO    + AnsiString::StringOfChar(' ', 6-seLOTNO.Length());
	sBuf += " Error：" + sErrLog + "(" + sePrg + ")";

	ServiceTkssS01->LogMessage(sBuf,EVENTLOG_INFORMATION_TYPE,0,0);
