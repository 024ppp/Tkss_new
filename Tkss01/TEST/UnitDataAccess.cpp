...
// 本来素材のみで扱うものだが、グラフを書くときにORテーブルが必要なため外に出した
// k.y 2003.05.17
//HANDLE hSOZAI_OR_DATA;

// k.y 2003.05.17
//KSM02_DATA *pSOZAI_OR_DATA;         // 素材のＯＲデータ
...
// k.y 2003.05.17
//HANDLE		hSPACE_DATA;  			//SKB_NO = 9 スペース読取専用

// k.y 2003.05.17
//KSM02_DATA  *pSPACE_DATA;
...
/* k.y 2003.05.17
    ...
    hSYUKA_DATA = NULL;
    // メモリの確保
    hSYUKA_DATA = GlobalAlloc( GHND, sizeof(struct KSM02_DATA) * 100 );
    if( hSYUKA_DATA == NULL ){
    ...
    pSYUKA_DATA = (struct KSM02_DATA *)GlobalLock(hSYUKA_DATA);
    if( hSYUKA_DATA == NULL ){
...
//得意先品番をキーにして必要ロットの取得をする（履歴数分）
    ...
    if( SGr_tSOKU->Cells[0][0].Length() == 5 ){
	    // 国内ロット（5桁）に対応
    	sBuf += " AND SUBSTR(LOTNO,2) < '" + SGr_tSOKU->Cells[0][0].SubString(2,5) + "'";
		sBuf += " ORDER BY SUBSTR(LOTNO,2) DESC";
	} else {
	    // 海外ロット（6桁）に対応
    	sBuf += " AND LOTNO < '" + SGr_tSOKU->Cells[0][0] + "'";
		sBuf += " ORDER BY LOTNO DESC";
	}
...
//ロットの抽出
    if( Query1->Eof == false ){
		...
		//2003.05.09 E.Takase Add↑
		//2003.05.09 E.Takase Add コメント↓
	    //iLOT_CNT = Edt_RIRE_NUM->Text.ToIntDef(10);
		//2003.05.09 E.Takase Add コメント↑
...
//  機能説明
//    ①ORテーブルを作成します。
//    ②出荷（測定）のデータを読み込み表示します。
//    ③出荷（履歴）、素材（測定）、素材（履歴）読込／表示関数を呼び出します。
//  パラメタ説明
//    なし。
//
//  戻り値
...
//
//---------------------------------------------------------------------------
void __fastcall TForm1::Set_Ins_Disp(void)                      //onishi
{
...
// ②KSM02(主)+KSD02(副)でラベル表示（左端の動的ラベル）が作成されるのではなく（←現在の処理）
//   測定Grid右側の左端ロットKSD02orKSM02(主)+KSD02(副)が最優先...
//　【②は「現在の処理」にした方が処理がシンプル＆現在問題となっていないので《対応しない方が良い》
//　　と、このコメントを書いている時の判断でした。＝②の対策は不要!! 】
        if( Query1->Eof == false )
        {
...
//---------------------------------------------------------------------------
bool __fastcall TForm1::Set_DB_SOKU(void)                           // onishi
{
    int i1,i2,i3,i4;
    //int nDRAW_GRID;
    //int nKENSAKU_KEY;
	...
	strcpy(pSetJissekiData[0].HINBAN	,pSOKU_KSD01_DATA[i2].HINBAN);
...
///////////////////////
	        ...
	        sBuf += " HINBAN, DTKSCOD, DTKSNAM, ZISNAM, HINMEI,";
	        ...
	        sBuf += AnsiString(pSetJissekiData[0].HINBAN) 		+ "','";
		...
		sBuf += " LOTNO5_SZ ='" 	+ AnsiString(pSetJissekiData[0].LOTNO5_SZ)     + "',";	//	得意先品番
	    	...
	    	sBuf += " HINBAN ='"		+ AnsiString(pSetJissekiData[0].HINBAN)	       + "',";	//	得意先品番
	...
	else
	{
	    	...
	    	else
		{
				...
				break;
                            case 2:						//良、否
                                try{
                                    ...
                                    //iRESULT = AnsiString(pCELLINFO_SOKU_SYUKA[iCol*iRowCount+iRow+i3].RESULT).ToIntDef(0);
			...
			switch( VALUE_TYPE )
    			{
	    		    	...
	    		    	if( bRtn && AnsiString(pCELLINFO_SOKU_SYUKA[iCol*iRowCount+iRow+i3].SOKU_FLG).ToIntDef(0) != 0)
		    		{
				    	...
				    	//iDISP_RESULT = AnsiString(pCELLINFO_SOKU_SYUKA[iCol*iRowCount+iRow+i3].DISP_RESULT).ToIntDef(0);
			        ...
			        //2003.05.19 E.Takase Add ↓
	    		        ...
	    		        //2003.05.19 E.Takase Add ↑
    		    	        //2003.05.19 E.Takase Add コメント↓
	    		        //DISP_RESULT_BUF = AnsiString(pCELLINFO_SOKU_SYUKA[iCol*iRowCount+iRow+i3].RESULT);
		    	        //2003.05.19 E.Takase Add コメント↑
                                if(AnsiString(pCELLINFO_SOKU_SYUKA[iCol*iRowCount+iRow+i3].SOKU_FLG).ToIntDef(0) != 0)
                                {
                           	    DISP_VAL_SU ++;
	    		    	...
	    		    	if( !bHANTEI ) RESULT_BUF 		= "";
                                break;
                            /* 2003/11/5 A.Tamura Comment
                            case 2:
	    			if( bHANTEI ) RESULT_BUF 		= AnsiString(iRESULT);
		    		if( !bHANTEI ) RESULT_BUF 		= "";
                                break;
                            */
                            default:
				...
				if( !bDISP_HANTEI ) DISP_RESULT_BUF 		= "";
                            ...
                            /* 2003/11/5 A.Tamura Comment
                            case 2:
		        	if( bDISP_HANTEI ) DISP_RESULT_BUF 		= AnsiString(iDISP_RESULT);
			        if( !bDISP_HANTEI ) DISP_RESULT_BUF 		= "";
                                break;
                            */
                            default:
				...
				case 7:
			            ...
			            if( bNEW == true )
				    {
			        	...
			        	sBuf = "insert into KSD03(";
    	                		sBuf += " DTKSHIN, LOTNO, UNQ_NO, SOKU_NO, SOKU_VAL, DISP_VAL";
        	    	        	sBuf += " )";
		            	        sBuf += " VALUES('";
        	    ...
        	    strcpy(SetData.DTKSHIN		,pCELLINFO_SOKU_SYUKA[iCol*iRowCount+iRow].DTKSHIN);
                    ...
                    //strcpy(SetData.SOKU_SU	    ,pCELLINFO_SOKU_SYUKA[iCol*iRowCount+iRow].SOKU_SU);
    		    ...
    		    //strcpy(SetData.SOKU_VAL_SU  ,AnsiString(SOKU_VAL_SU).c_str());
                    //指示数と実績数を同じにする
                    //strcpy(SetData.SOKU_VAL_SU	,pCELLINFO_SOKU_SYUKA[iCol*iRowCount+iRow].SOKU_SU);
                    strcpy(SetData.SOKU_VAL_SU	    ,AnsiString(iSIJI_NSU).c_str());
           	    //if( SOKU_VAL_SU > 0)
	            //{
		    //	SOKU_VAL_AVE = GOUKEI_RESULT / SOKU_VAL_SU;
		    //}
                    //else
    	    	    //{
	    	    //	SOKU_VAL_AVE = 0;
		    ...
		    //}
        	    //else
	            //{
		    //	 strcpy(SetData.SOKU_OK	,"1");
		    //}
                    strcpy(SetData.SOKU_OK,AnsiString(iOK_FLG).c_str());
                    //指示数と実績数を同じにする
                    //strcpy(SetData.DISP_VAL_SU	,pCELLINFO_SOKU_SYUKA[iCol*iRowCount+iRow].SOKU_SU);
                    strcpy(SetData.DISP_VAL_SU	    ,AnsiString(iSIJI_NSU).c_str());
        	    //if( DISP_VAL_SU > 0)
	            //{
		    //	DISP_VAL_AVE = DISP_GOUKEI_RESULT / DISP_VAL_SU;
		    //}
    		    //else
    	    	    //{
	    	    //	DISP_VAL_AVE = 0;
			...
			if( bNEW == true )										//新規登録
			{
			    ...
			    strcpy(SetData.UPDCNT,pCELLINFO_SOKU_SYUKA[iCol*iRowCount+iRow].UPDCNT);
    			    ...
    			    sBuf = "insert into KSD02(";
            		    sBuf += " DTKSHIN, LOTNO, UNQ_NO, ORDER_NO, SKB_NO, HINBAN_SK, KOMK_NO, KOMK_NM,";
...
//KSD01の更新
    ...
    for(i2 = 0;i2 < SGr_vRIRE_SYUKA->ColCount;i2++)
    {
		...
		//2003.05.09 E.Takase Add↑
		//2003.05.09 E.Takase Add コメント↓
    	//strcpy(pSetJissekiData[0].DTKSHIN,Cmb_DTKSHIN->Text.c_str());
		//2003.05.09 E.Takase Add コメント↑
		...
		strcpy(pSetJissekiData[0].HINBAN	,pRIRE_KSD01_DATA[i2].HINBAN);
		...
		//2003.05.19 E.Takase Add ↑
		//2003.05.19 E.Takase Add コメント↓
		//sMEMO = Form1->Mem_vRIRE_MEMO[i2]->Text;
		//2003.05.19 E.Takase Add コメント↑
		if(sMEMO.Length() == 0 ) sMEMO = " ";
		strcpy(pSetJissekiData[0].MEMO			,sMEMO.c_str());
	...
	for(iCol = 0; iCol < SGr_vRIRE_SYUKA->ColCount;iCol++)
	{
		...
		else
        {
                    ...
                    case 2:						//良、否
    					if( iDEBUG != 1)
	    				{
    	            		try{
                                // 2003/11/5 A.Tamura
                                // 良/否判定も未入力のまま保存させたいので RESULT_BUF を使って処理する
                                //iRESULT = AnsiString(pCELLINFO_RIRE_SYUKA[iCol*iRowCount+iRow+i3].RESULT).ToIntDef(0);
                    ...
                    case 2:						//良、否
                        if( iDEBUG != 1)
		    			{
	                    	try{
                                // 2003/11/5 A.Tamura
                                // 良/否判定も未入力のまま保存させたいので DISP_RESULT_BUF を使って処理する
				    	    	//iDISP_RESULT = AnsiString(pCELLINFO_RIRE_SYUKA[iCol*iRowCount+iRow+i3].DISP_RESULT).ToIntDef(0);
                 	...
                 	case 3:
		    			...
		    			//2003.05.19 E.Takase Add ↑
			    		//2003.05.19 E.Takase Add コメント↓
				    	//DISP_RESULT_BUF = AnsiString(pCELLINFO_RIRE_SYUKA[iCol*iRowCount+iRow+i3].RESULT);
					    //2003.05.19 E.Takase Add コメント↑
                        if(AnsiString(pCELLINFO_RIRE_SYUKA[iCol*iRowCount+iRow+i3].SOKU_FLG).ToIntDef(0)!=0)
                        {
                       	    DISP_VAL_SU ++;
            ...
            //１項目のセーブ実行
                ...
                //strcpy(SetData.SOKU_SU		,pCELLINFO_RIRE_SYUKA[iCol*iRowCount+iRow].SOKU_SU);
                strcpy(SetData.SOKU_SU		,AnsiString(iSIJI_NSU).c_str());
		    	...
		    	//strcpy(SetData.SOKU_VAL_SU  ,AnsiString(SOKU_VAL_SU).c_str());
                //strcpy(SetData.SOKU_VAL_SU	,pCELLINFO_RIRE_SYUKA[iCol*iRowCount+iRow].SOKU_SU);
                strcpy(SetData.SOKU_VAL_SU		,AnsiString(iSIJI_NSU).c_str());
    			if( iSOKU_VAL_SU > 0) {
	    			SOKU_VAL_AVE = GOUKEI_RESULT / iSOKU_VAL_SU;
		    	}
                ...
                //strcpy(SetData.DISP_VAL_SU	,pCELLINFO_RIRE_SYUKA[iCol*iRowCount+iRow].SOKU_SU);
                strcpy(SetData.DISP_VAL_SU		,AnsiString(iSIJI_NSU).c_str());
    			if( iDISP_VAL_SU > 0) {
	    			DISP_VAL_AVE = DISP_GOUKEI_RESULT / iDISP_VAL_SU;
		    	}
        ...
        else
        {
	    	for( iRow = 1;iRow < iRowCount; iRow=iRow)
		    {
                        ...
                        case 2:						//良、否
        					if(iDEBUG != 1)
		        			{
	                    		try{
                                	// 2003/11/5 A.Tamura
                                	// 良/否判定も未入力のまま保存させたいので RESULT_BUF を使って処理する
                                	//iRESULT = AnsiString(pCELLINFO_SOKU_SOZAI[iCol*iRowCount+iRow+i3].RESULT).ToIntDef(0);
                        ...
                        case 2:						//良、否
        					if( iDEBUG != 1 )
		        			{
	                    		try{
                                	// 2003/11/5 A.Tamura
                                	// 良/否判定も未入力のまま保存させたいので DISP_RESULT_BUF を使って処理する
                                	//iDISP_RESULT = AnsiString(pCELLINFO_SOKU_SOZAI[iCol*iRowCount+iRow+i3].DISP_RESULT).ToIntDef(0);
             	        ...
             	        case 3:
				        	...
				        	//2003.05.19 E.Takase Add ↑
        					//2003.05.19 E.Takase Add コメント↓
		        			//DISP_RESULT_BUF = AnsiString(pCELLINFO_SOKU_SOZAI[iCol*iRowCount+iRow+i3].RESULT);
				        	//2003.05.19 E.Takase Add コメント↑
                            if(AnsiString(pCELLINFO_SOKU_SOZAI[iCol*iRowCount+iRow+i3].SOKU_FLG).ToIntDef(0)!=0)
                            {
                           	    DISP_VAL_SU ++;
                ...
                //１項目のセーブ実行
                    ...
                    //strcpy(SetData.SOKU_SU		,pCELLINFO_SOKU_SOZAI[iCol*iRowCount+iRow].SOKU_SU);
                    strcpy(SetData.SOKU_SU		,AnsiString(iSIJI_NSU).c_str());
        			...
        			//strcpy(SetData.SOKU_VAL_SU  ,AnsiString(SOKU_VAL_SU).c_str());
                    //strcpy(SetData.SOKU_VAL_SU	,pCELLINFO_SOKU_SOZAI[iCol*iRowCount+iRow].SOKU_SU);
                    strcpy(SetData.SOKU_VAL_SU		,AnsiString(iSIJI_NSU).c_str());
		        	//if( SOKU_VAL_SU > 0) {
        			//	SOKU_VAL_AVE = GOUKEI_RESULT / SOKU_VAL_SU;
		        	//}
                    ...
                    strcpy(SetData.SOKU_OK,AnsiString(iOK_FLG).c_str());
		        	//strcpy(SetData.DISP_VAL_SU  ,AnsiString(DISP_VAL_SU).c_str());
                    //strcpy(SetData.DISP_VAL_SU	  ,pCELLINFO_SOKU_SOZAI[iCol*iRowCount+iRow].SOKU_SU);
                    strcpy(SetData.DISP_VAL_SU	   ,AnsiString(iSIJI_NSU).c_str());
        			if( iDISP_VAL_SU > 0) {
		        		DISP_VAL_AVE = DISP_GOUKEI_RESULT / iDISP_VAL_SU;
			        }
                        ...
                        case 2:						//良、否
                            if(iDEBUG != 1)
				        	{
						        try{
                                	// 2003/11/5 A.Tamura
                                	// 良/否判定も未入力のまま保存させたいので RESULT_BUF を使って処理する
                                	//iRESULT = AnsiString(pCELLINFO_RIRE_SOZAI[iCol*iRowCount+iRow+i3].RESULT).ToIntDef(0);
                        ...
                        case 2:						//良、否
        					if( iDEBUG != 1)
		        			{
	                    		try{
                                	// 2003/11/5 A.Tamura
                                	// 良/否判定も未入力のまま保存させたいので DISP_RESULT_BUF を使って処理する
						        	//iDISP_RESULT = AnsiString(pCELLINFO_RIRE_SOZAI[iCol*iRowCount+iRow+i3].DISP_RESULT).ToIntDef(0);
             	        ...
             	        case 3:
				        	...
				        	//2003.05.19 E.Takase Add ↑
        					//2003.05.19 E.Takase Add コメント↓
		        			//DISP_RESULT_BUF = AnsiString(pCELLINFO_RIRE_SOZAI[iCol*iRowCount+iRow+i3].RESULT);
				        	//2003.05.19 E.Takase Add コメント↑
                           	if(AnsiString(pCELLINFO_RIRE_SOZAI[iCol*iRowCount+iRow+i3].SOKU_FLG).ToIntDef(0)!=0)
                            {
                                DISP_VAL_SU ++;
...
//１項目のセーブ実行
            ...
            //strcpy(SetData.SOKU_SU		,pCELLINFO_RIRE_SOZAI[iCol*iRowCount+iRow].SOKU_SU);
            strcpy(SetData.SOKU_SU		,AnsiString(iSIJI_NSU).c_str());
			...
			//strcpy(SetData.SOKU_VAL_SU  ,AnsiString(SOKU_VAL_SU).c_str());
            //strcpy(SetData.SOKU_VAL_SU	,pCELLINFO_RIRE_SOZAI[iCol*iRowCount+iRow].SOKU_SU);
            strcpy(SetData.SOKU_VAL_SU		,AnsiString(iSIJI_NSU).c_str());
			//if( SOKU_VAL_SU > 0) {
			//	SOKU_VAL_AVE = GOUKEI_RESULT / SOKU_VAL_SU;
			//}
...
/*
    ...
    if( hSetJissekiData ){
    	GlobalUnlock( hSetJissekiData );
        hSetJissekiData = GlobalFree( hSetJissekiData );
    }
*/
    return(true);
}
...
/* k.y 2003.05.17
        ...
        hRIREKI_DATA = NULL;
        // メモリの確保
        hRIREKI_DATA = GlobalAlloc( GHND, sizeof(struct KSD02_DATA) * 100 );	//項目最大数100件と過程
...
//出荷検査のグリッドをすべて灰色にする
		for(i2= 1; i2 <  SGr_vSOKU_SYUKA->RowCount ; i2++)
        {
			...
			//strcpy(pCELLINFO_SOKU_SYUKA[(iCol * SGr_vSOKU_SYUKA->RowCount) + i2].RESULT,AnsiString("不要").c_str());
			//strcpy(pCELLINFO_SOKU_SYUKA[(iCol * SGr_vSOKU_SYUKA->RowCount) + i2].DISP_RESULT,AnsiString("不要").c_str());
...
//測定機器NO、測定項目NOの一致している場所を探索
        ...
        for( i1 = 0; i1 < i4;i1++)
        {
            ...
            while( 1 )
            {
                ...
                //2004.04.20 E.Takase 修正 一致条件変更（単位も追加）
                //測定項目名+測定機器NO → 測定項目名+測定機器NO+単位
				//2004.04.20 E.Takase Add↓
...
//履歴データのデータ読み取り
            ...
            for(i3 = 0; i3 <  AnsiString(pRIREKI_DATA[i1].SOKU_SU).ToIntDef(0) ; i3++ )
            {
				...
				//2003.05.17 E.Takase Add↑
				//2003.05.17 E.Takase Add コメント↓
		  		//pCELLINFO_SOKU_SYUKA[(iCol * SGr_vSOKU_SYUKA->RowCount) + iRow].UPDNAME = pRIREKI_DATA[i1].UPDNAME;
				//2003.05.17 E.Takase Add コメント↑
				//2003.05.10 E.Takase Add↑
				//2003.05.10 E.Takase Add コメント↓
		  		//pCELLINFO_SOKU_SYUKA[(iCol * SGr_vSOKU_SYUKA->RowCount) + iRow].UPDNAME = GetTANTO(AnsiString(pRIREKI_DATA[i1].UPDCHR));
				//2003.05.10 E.Takase Add コメント↑
				//規格
            ...
            for(i3 = 0; i3 <  AnsiString(pRIREKI_DATA[i1].SOKU_SU).ToIntDef(0) ; i3++ )
            {
                ...
                {
                case 0:
//                	SGr_vSOKU_SYUKA->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL01;
                    ...
                    break;
                case 1:
//                	SGr_vSOKU_SYUKA->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL02;
                    ...
                    break;
                case 2:
//                	SGr_vSOKU_SYUKA->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL03;
                    ...
                    break;
                case 3:
//                	SGr_vSOKU_SYUKA->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL04;
                    ...
                    break;
                case 4:
//                	SGr_vSOKU_SYUKA->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL05;
                    ...
                    break;
                case 5:
//                	SGr_vSOKU_SYUKA->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL06;
                    ...
                    break;
                case 6:
//                	SGr_vSOKU_SYUKA->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL07;
                    ...
                    break;
                case 7:
//                	SGr_vSOKU_SYUKA->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL08;
                    ...
                    break;
                case 8:
//                	SGr_vSOKU_SYUKA->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL09;
                    ...
                    break;
                case 9:
//                	SGr_vSOKU_SYUKA->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL10;
                ...
                default:
    				...
    				sBuf += " AND LOTNO = '"	 + AnsiString(pCELLINFO_SOKU_SYUKA[(iCol * SGr_vSOKU_SYUKA->RowCount) + iRow].LOTNO) 	+ "'";                        //出荷検査項目、材料証明項目
    				sBuf += " AND UNQ_NO = "	 + AnsiString(pCELLINFO_SOKU_SYUKA[(iCol * SGr_vSOKU_SYUKA->RowCount) + iRow].UNQ_NO) 	+ " ";                        //出荷検査項目、材料証明項目
    				//sBuf += " AND SOKU_NO = "	 + AnsiString(i3+1) 	+ " ";                        //出荷検査項目、材料証明項目
...
//////////////////////////////////////////////////////////////
	while( 1 )
	{
    	//if(i1 >= SGr_vRIRE_SYUKA->RowCount ) break;
		//iUNQNO = AnsiString(pCELLINFO_SOKU_SYUKA[i1].UNQ_NO).ToIntDef(0);
		//sUNQNO += " " + AnsiString(pCELLINFO_SOKU_SYUKA[i1].UNQ_NO) + " " + ",";
		//while(iUNQNO == AnsiString(pCELLINFO_SOKU_SYUKA[i1].UNQ_NO).ToIntDef(0))
		//{
		//	i1++;
		//	if(i1 >= SGr_vRIRE_SYUKA->RowCount ) break;
		//}
    	//if(i1 >= SGr_vRIRE_SYUKA->RowCount ) break;
		//iUNQNO = AnsiString(pCELLINFO_RIRE_SYUKA[i1].KOMK_NO).ToIntDef(0);
		//sUNQNO += " " + AnsiString(pCELLINFO_RIRE_SYUKA[i1].KOMK_NO) + " " + ",";
		//while(iUNQNO == AnsiString(pCELLINFO_RIRE_SYUKA[i1].KOMK_NO).ToIntDef(0))
		//{
		//	i1++;
		//	if(i1 >= SGr_vRIRE_SYUKA->RowCount ) break;
		//}
...
//出荷検査のグリッドをすべて灰色にする
		for(i2= 1; i2 <  SGr_vSOKU_SYUKA->RowCount ; i2++)
        {
			...
			//strcpy(pCELLINFO_RIRE_SYUKA[(iCol * SGr_vRIRE_SYUKA->RowCount)+i2].RESULT,AnsiString("不要").c_str());	//項目NO=0なら項目が無い
			//strcpy(pCELLINFO_RIRE_SYUKA[(iCol * SGr_vRIRE_SYUKA->RowCount)+i2].DISP_RESULT,AnsiString("不要").c_str());	//項目NO=0なら項目が無い
...
//測定機器NO、測定項目NOの一致している場所を探索
        ...
        for( i1 = 0; i1 < i4;i1++)
        {
            ...
            //2003.05.08 E.Takase Add↓
            //担当者取得
            //sBufTANTO = GetTANTO(AnsiString(pRIREKI_DATA[i1].UPDCHR));
            //2003.05.08 E.Takase Add↑
			//2003.05.10 E.Takase Add コメント↑
//履歴データのデータ読み取り
            iTempRow = iRow;
            ...
            for(i3 = 0; i3 <  AnsiString(pRIREKI_DATA[i1].SOKU_SU).ToIntDef(0) ; i3++ )
            {
				...
				//2003.05.17 E.Takase Add↑
				//2003.05.17 E.Takase Add コメント↓
                //pCELLINFO_RIRE_SYUKA[(iCol * SGr_vRIRE_SYUKA->RowCount) + iRow].UPDNAME =  pRIREKI_DATA[i1].UPDNAME;
				//2003.05.17 E.Takase Add コメント↑
               	//2003.05.10 E.Takase Add↑
                //2003.05.10 E.Takase Add コメント↓
                //pCELLINFO_RIRE_SYUKA[(iCol * SGr_vRIRE_SYUKA->RowCount) + iRow].UPDNAME = GetTANTO(AnsiString(pRIREKI_DATA[i1].UPDCHR));
                //2003.05.10 E.Takase Add コメント↑
				//規格
            ...
            for(i3 = 0; i3 <  AnsiString(pRIREKI_DATA[i1].SOKU_SU).ToIntDef(0) ; i3++ )
            {
                ...
                {
                case 0:
//                	SGr_vRIRE_SYUKA->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL01;
                    ...
                    break;
                case 1:
//                	SGr_vRIRE_SYUKA->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL02;
                    ...
                    break;
                case 2:
//                	SGr_vRIRE_SYUKA->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL03;
                    ...
                    break;
                case 3:
//                	SGr_vRIRE_SYUKA->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL04;
                    ...
                    break;
                case 4:
//                	SGr_vRIRE_SYUKA->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL05;
                    ...
                    break;
                case 5:
//                	SGr_vRIRE_SYUKA->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL06;
                    ...
                    break;
                case 6:
//                	SGr_vRIRE_SYUKA->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL07;
                    ...
                    break;
                case 7:
//                	SGr_vRIRE_SYUKA->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL08;
                    ...
                    break;
                case 8:
//                	SGr_vRIRE_SYUKA->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL09;
                    ...
                    break;
                case 9:
//                	SGr_vRIRE_SYUKA->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL10;
...
//現在未使用です
//----------------------------------------
void __fastcall TForm1::Set_SOZAIJISSEKI(void)
{
    ...
    AnsiString sBuf,sBuf1;
//出荷素材測定の測定結果の読み込み
	AnsiString sLOTNO,sKOMK_NO,sKOMK_NM,sDTKSHIN,sDTKSHIN_SZ,sHINBAN_SK;
	int iLotChange;
	int iLoop;
	bool bSET_SOZAI;
	...
	for(i1 = 0;i1 < SGr_tSOKU->ColCount ; i1++)
	{
        if(SGr_tSOKU->Cells[i1][0].Length() != 0)
        {
		    sLot += "'" + SGr_tSOKU->Cells[i1][0] + "',";
        }
	}
...
///////////////////
//	過去にデータがある場合は灰色表示なのでSKBNO2,3についても調べる
//////////////////////
	sBuf  = "SELECT LOTNO FROM KSD02";
    sBuf += " WHERE DTKSHIN = '" + sNowDTKSHIN + "'";
    sBuf += " AND LOTNO IN ("+ sLot + ")";
    sBuf += " ORDER BY LOTNO";
//For Debug
...
//履歴データが有る出荷ロットが呼び出されているはず
    ...
    {
        sBuf = Query3->FieldValues["LOTNO"];
        for(iCol = 0;iCol < SGr_tSOKU->RowCount ; iCol++ )
		{
			if(SGr_tSOKU->Cells[iCol][0].Trim() == sBuf.Trim())
			{
//出荷検査のグリッドをすべて灰色にする
				for(i2= 1; i2 <  SGr_vSOKU_SOZAI->RowCount ; i2++)
        		{
            		//strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + i2].RESULT		, AnsiString("不要").c_str());
            		//strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + i2].DISP_RESULT, AnsiString("不要").c_str());
            		strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + i2].RESULT		, AnsiString("").c_str());
            		strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + i2].DISP_RESULT, AnsiString("").c_str());
            		strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + i2].KOMK_NO	, "0");
        		}
				break;
			}
		}
...
//履歴データが有る出荷ロットが呼び出されているはず
    ...
    {
    	i1 = 0;
        sLot = Query3->FieldValues["LOTNO"];
//ロット毎にメモリの初期化が必要
        memset(pRIREKI_DATA,0x00,sizeof(pRIREKI_DATA));

        while( sLot == Query3->FieldValues["LOTNO"] && Query3->Eof != true)
        {
            strcpy(pRIREKI_DATA[i1].DTKSHIN		,"");                      //品番
            if ( VarIsNull(Query3->FieldValues["DTKSHIN"]) != true)
            {
	        	sBuf	= Query3->FieldValues["DTKSHIN"];							//得意先項目品番
    	        strcpy(pRIREKI_DATA[i1].DTKSHIN			,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].LOTNO		,"");                      //品番
			if ( VarIsNull(Query3->FieldValues["LOTNO"]) != true )
            {
            	sBuf = Query3->FieldValues["LOTNO"];                   				//ロットNO
            	strcpy(pRIREKI_DATA[i1].LOTNO      		,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].UNQ_NO		,"");                      //品番
			if ( VarIsNull(Query3->FieldValues["UNQ_NO"]) != true)
            {
            	sBuf = Query3->FieldValues["UNQ_NO"];                   			//明細NO
            	strcpy(pRIREKI_DATA[i1].UNQ_NO      		,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].ORDER_NO		,"");                      //品番
			if ( VarIsNull(Query3->FieldValues["ORDER_NO"]) != true)
            {
            	sBuf = Query3->FieldValues["ORDER_NO"];                   		  	//表示・印刷順
            	strcpy(pRIREKI_DATA[i1].ORDER_NO      		,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].SKB_NO		,"");                      //品番
			if ( VarIsNull(Query3->FieldValues["SKB_NO"]) != true )
            {
            	sBuf = Query3->FieldValues["SKB_NO"];                   		 	//識別NO
            	strcpy(pRIREKI_DATA[i1].SKB_NO      		,sBuf.c_str());
			}
            strcpy(pRIREKI_DATA[i1].HINBAN_SK		,"");                      //品番
            if ( VarIsNull(Query3->FieldValues["HINBAN_SK"]) != true)
            {
            	sBuf	= Query3->FieldValues["HINBAN_SK"];
                strcpy(pRIREKI_DATA[i1].HINBAN_SK	,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].KOMK_NO		,"");                      //品番
            if ( VarIsNull(Query3->FieldValues["KOMK_NO"]) != true )
            {
	            sBuf    = Query3->FieldValues["KOMK_NO"];                           //測定項目NO
    	        strcpy(pRIREKI_DATA[i1].KOMK_NO			,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].KOMK_NM,"");                 				//測定項目名称
            if ( VarIsNull(Query3->FieldValues["KOMK_NM"]) != true )
            {
            	sBuf = Query3->FieldValues["KOMK_NM"];
                strcpy(pRIREKI_DATA[i1].KOMK_NM		,sBuf.c_str());
            }

            strcpy(pRIREKI_DATA[i1].KOMK_ENG,"");                 				//測定項目名称（英語)
            if ( VarIsNull(Query3->FieldValues["KOMK_ENG"]) != true )
            {
            	sBuf = Query3->FieldValues["KOMK_ENG"];
                strcpy(pRIREKI_DATA[i1].KOMK_ENG		,sBuf.c_str());
            }

            strcpy(pRIREKI_DATA[i1].KOMK_SUB,"");                     			//測定項目サブ名称
            if ( VarIsNull(Query3->FieldValues["KOMK_SUB"]) != true )
            {
            	sBuf = Query3->FieldValues["KOMK_SUB"];
                strcpy(pRIREKI_DATA[i1].KOMK_SUB	,sBuf.c_str());
            }

            strcpy(pRIREKI_DATA[i1].KOMK_SUB_ENG,"");                     	    //測定項目サブ名称(英語)
            if ( VarIsNull(Query3->FieldValues["KOMK_SUB_ENG"]) != true)
            {
            	sBuf = Query3->FieldValues["KOMK_SUB_ENG"];
                strcpy(pRIREKI_DATA[i1].KOMK_SUB_ENG	,sBuf.c_str());
            }

            strcpy(pRIREKI_DATA[i1].KIKI_NO,"");                     	    //測定項目サブ名称(英語)
            if ( VarIsNull(Query3->FieldValues["KIKI_NO"]) != true )
            {
	            sBuf    = Query3->FieldValues["KIKI_NO"];                       	//測定機器NO
    	        strcpy(pRIREKI_DATA[i1].KIKI_NO			,sBuf.c_str());
         	}
            strcpy(pRIREKI_DATA[i1].KIKI_NM,"");
            if ( VarIsNull(Query3->FieldValues["KIKI_NM"]) != true )				//測定機器名称
            {
            	sBuf = Query3->FieldValues["KIKI_NM"];
                strcpy(pRIREKI_DATA[i1].KIKI_NM		,sBuf.c_str());
            }

            strcpy(pRIREKI_DATA[i1].TANI       ,"");                    		//単位
            if ( VarIsNull(Query3->FieldValues["TANI"]) != true )
			{
            	sBuf       = Query3->FieldValues["TANI"];
            	strcpy(pRIREKI_DATA[i1].TANI       ,sBuf.c_str());
			}                                                           		//規格タイプ

            strcpy(pRIREKI_DATA[i1].KIKA_TYPE       ,"");                    		//単位
            if ( VarIsNull(Query3->FieldValues["KIKA_TYPE"]) != true )
			{
	            sBuf  = Query3->FieldValues["KIKA_TYPE"];
    	        strcpy(pRIREKI_DATA[i1].KIKA_TYPE  ,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].KIKA_KIND       ,"");                    		//単位
            if ( VarIsNull(Query3->FieldValues["KIKA_KIND"]) != true )
			{
            	sBuf  = Query3->FieldValues["KIKA_KIND"];              				//規格種類
            	strcpy(pRIREKI_DATA[i1].KIKA_KIND  ,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].KIKA_DECI       ,"");                    		//単位
            if ( VarIsNull(Query3->FieldValues["KIKA_DECI"]) != true )
 			{
            	sBuf  = Query3->FieldValues["KIKA_DECI"];                         	//規格小数点以下桁数
            	strcpy(pRIREKI_DATA[i1].KIKA_DECI  ,sBuf.c_str());
			}
            strcpy(pRIREKI_DATA[i1].KIKA_NUM1       ,"");                    		//単位
            if ( VarIsNull(Query3->FieldValues["KIKA_NUM1"]) != true )
			{
            	sBuf  = Query3->FieldValues["KIKA_NUM1"];                         	//規格値１
            	strcpy(pRIREKI_DATA[i1].KIKA_NUM1  ,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].KIKA_NUM2       ,"");                    		//単位
            if ( VarIsNull(Query3->FieldValues["KIKA_NUM2"]) != true )
			{
            	sBuf  = Query3->FieldValues["KIKA_NUM2"];                      		//規格値２
            	strcpy(pRIREKI_DATA[i1].KIKA_NUM2  ,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].KIKA_NUM3       ,"");                    		//単位
            if ( VarIsNull(Query3->FieldValues["KIKA_NUM3"]) != true )
			{
            	sBuf  = Query3->FieldValues["KIKA_NUM3"];                       	//規格値３
            	strcpy(pRIREKI_DATA[i1].KIKA_NUM3  ,sBuf.c_str());
			}
            strcpy(pRIREKI_DATA[i1].KIKA_STR       ,"");                    		//単位
            if ( VarIsNull(Query3->FieldValues["KIKA_STR"]) != true  )
			{
            	sBuf   = Query3->FieldValues["KIKA_STR"];                        	//規格値文字
            	strcpy(pRIREKI_DATA[i1].KIKA_STR   ,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].KIKA_SUB1       ,"");                    		//単位
            if ( VarIsNull(Query3->FieldValues["KIKA_SUB1"]) != true )
			{
            	sBuf  = Query3->FieldValues["KIKA_SUB1"];                       	//規格値サブ１
            	strcpy(pRIREKI_DATA[i1].KIKA_SUB1  ,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].KIKA_SUB2       ,"");                    		//単位
            if ( VarIsNull(Query3->FieldValues["KIKA_SUB2"]) != true )
			{
            	sBuf  = Query3->FieldValues["KIKA_SUB2"];                         	//規格値サブ２
            	strcpy(pRIREKI_DATA[i1].KIKA_SUB2  ,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].KIKA_SUB3       ,"");                    		//単位
            if ( VarIsNull(Query3->FieldValues["KIKA_SUB3"]) != true )
			{
            	sBuf  = Query3->FieldValues["KIKA_SUB3"];                        	//規格値サブ３
            	strcpy(pRIREKI_DATA[i1].KIKA_SUB3  ,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].SOKU_SU       ,"");                    		//単位
            if ( VarIsNull(Query3->FieldValues["SOKU_SU"]) != true  )
			{
            	sBuf    = Query3->FieldValues["SOKU_SU"];                   		//測定指示ｎ数
            	strcpy(pRIREKI_DATA[i1].SOKU_SU    ,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].SOKU_FLG       ,"");                    		//単位
            if ( VarIsNull(Query3->FieldValues["SOKU_FLG"]) != true )
			{
            	sBuf    = Query3->FieldValues["SOKU_FLG"];                   		//測定項目の有無
            	strcpy(pRIREKI_DATA[i1].SOKU_FLG    ,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].TOKU_FLG       ,"");                    		//単位
            if ( VarIsNull(Query3->FieldValues["TOKU_FLG"]) != true )
			{
	            sBuf    = Query3->FieldValues["TOKU_FLG"];                   		//特別項目の有無
	            strcpy(pRIREKI_DATA[i1].TOKU_FLG    ,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].KIKA_PRT       ,"");                    		//単位
            if ( VarIsNull(Query3->FieldValues["KIKA_PRT"]) != true )
			{
            	sBuf    = Query3->FieldValues["KIKA_PRT"];                   		//上限下限値の印刷
            	strcpy(pRIREKI_DATA[i1].KIKA_PRT    ,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].VALUE_TYPE       ,"");                    		//単位
            if ( VarIsNull(Query3->FieldValues["VALUE_TYPE"]) != true)
			{
            	sBuf    = Query3->FieldValues["VALUE_TYPE"];                   		//測定値表現
            	strcpy(pRIREKI_DATA[i1].VALUE_TYPE    ,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].VALUE_DECI       ,"");                    		//単位
            if ( VarIsNull(Query3->FieldValues["VALUE_DECI"]) != true)
			{
            	sBuf    = Query3->FieldValues["VALUE_DECI"];                   		//測定値小数点以下桁数
            	strcpy(pRIREKI_DATA[i1].VALUE_DECI    ,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].SKTYMD       ,"");                    		//単位
            if ( VarIsNull(Query3->FieldValues["SKTYMD"]) != true)
			{
            	sBuf     = Query3->FieldValues["SKTYMD"];                       	//測定日
            	strcpy(pRIREKI_DATA[i1].SKTYMD     ,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].ADDYMD       ,"");                    		//単位
            if ( VarIsNull(Query3->FieldValues["ADDYMD"]) != true)
			{
            	sBuf     = Query3->FieldValues["ADDYMD"];                      		//登録日
            	strcpy(pRIREKI_DATA[i1].ADDYMD     ,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].ADDTIM       ,"");                    		//単位
            if ( VarIsNull(Query3->FieldValues["ADDTIM"]) != true)
			{
            	sBuf     = Query3->FieldValues["ADDTIM"];                       	//登録時間
            	strcpy(pRIREKI_DATA[i1].ADDTIM     ,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].UPDYMD       ,"");                    		//単位
            if ( VarIsNull(Query3->FieldValues["UPDYMD"]) != true)
			{
            	sBuf     = Query3->FieldValues["UPDYMD"];                         	//変更日
            	strcpy(pRIREKI_DATA[i1].UPDYMD     ,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].UPDTIM       ,"");                    		//単位
            if ( VarIsNull(Query3->FieldValues["UPDTIM"]) != true )
			{
            	sBuf     = Query3->FieldValues["UPDTIM"];                          	//変更時間
            	strcpy(pRIREKI_DATA[i1].UPDTIM     ,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].UPDCHR       ,"");                    		//単位
            if ( VarIsNull(Query3->FieldValues["UPDCHR"]) != true )
			{
            	sBuf     = Query3->FieldValues["UPDCHR"];                      		//更新者
            	strcpy(pRIREKI_DATA[i1].UPDCHR     ,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].UPDCNT       ,"");                    		//単位
            if ( VarIsNull(Query3->FieldValues["UPDCNT"]) != true )
			{
            	sBuf     = Query3->FieldValues["UPDCNT"];                         	//更新回数
            	strcpy(pRIREKI_DATA[i1].UPDCNT     ,sBuf.c_str());
			}
//SOKU Result Read ///////////////////////////////////////////////////////////////////////
			strcpy(pRIREKI_DATA[i1].SOKU_VAL01,"");
            if(VarIsNull(Query3->FieldValues["SOKU_VAL01"])!=true)
            {
            	sBuf = Query3->FieldValues["SOKU_VAL01"];
                strcpy(pRIREKI_DATA[i1].SOKU_VAL01		,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].SOKU_VAL02,"");
            if(VarIsNull(Query3->FieldValues["SOKU_VAL02"])!=true)
            {
            	sBuf = Query3->FieldValues["SOKU_VAL02"];
                strcpy(pRIREKI_DATA[i1].SOKU_VAL02		,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].SOKU_VAL03,"");
            if(VarIsNull(Query3->FieldValues["SOKU_VAL03"])!=true)
            {
            	sBuf = Query3->FieldValues["SOKU_VAL03"];
                strcpy(pRIREKI_DATA[i1].SOKU_VAL03		,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].SOKU_VAL04,"");
            if(VarIsNull(Query3->FieldValues["SOKU_VAL04"])!=true)
            {
            	sBuf = Query3->FieldValues["SOKU_VAL04"];
                strcpy(pRIREKI_DATA[i1].SOKU_VAL04		,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].SOKU_VAL05,"");
            if(VarIsNull(Query3->FieldValues["SOKU_VAL05"])!=true)
            {
            	sBuf = Query3->FieldValues["SOKU_VAL05"];
                strcpy(pRIREKI_DATA[i1].SOKU_VAL05		,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].SOKU_VAL06,"");
            if(VarIsNull(Query3->FieldValues["SOKU_VAL06"])!=true)
            {
            	sBuf = Query3->FieldValues["SOKU_VAL06"];
                strcpy(pRIREKI_DATA[i1].SOKU_VAL06		,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].SOKU_VAL07,"");
            if(VarIsNull(Query3->FieldValues["SOKU_VAL07"])!=true)
            {
            	sBuf = Query3->FieldValues["SOKU_VAL07"];
                strcpy(pRIREKI_DATA[i1].SOKU_VAL07		,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].SOKU_VAL08,"");
            if(VarIsNull(Query3->FieldValues["SOKU_VAL08"])!=true)
            {
            	sBuf = Query3->FieldValues["SOKU_VAL08"];
                strcpy(pRIREKI_DATA[i1].SOKU_VAL08		,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].SOKU_VAL09,"");
            if(VarIsNull(Query3->FieldValues["SOKU_VAL09"])!=true)
            {
            	sBuf = Query3->FieldValues["SOKU_VAL09"];
                strcpy(pRIREKI_DATA[i1].SOKU_VAL09		,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].SOKU_VAL10,"");
            if(VarIsNull(Query3->FieldValues["SOKU_VAL10"])!=true)
            {
            	sBuf = Query3->FieldValues["SOKU_VAL10"];
                strcpy(pRIREKI_DATA[i1].SOKU_VAL10		,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].SOKU_VAL_SU,"1");
            if(VarIsNull(Query3->FieldValues["SOKU_VAL_SU"])!=true)
            {
            	sBuf = Query3->FieldValues["SOKU_VAL_SU"];
                strcpy(pRIREKI_DATA[i1].SOKU_VAL_SU		,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].SOKU_VAL_AVE,"0.0");
            if(VarIsNull(Query3->FieldValues["SOKU_VAL_AVE"])!=true)
            {
            	sBuf = Query3->FieldValues["SOKU_VAL_AVE"];
                strcpy(pRIREKI_DATA[i1].SOKU_VAL_AVE		,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].SOKU_VAL_MAX,"0.0");
            if(VarIsNull(Query3->FieldValues["SOKU_VAL_MAX"])!=true)
            {
            	sBuf = Query3->FieldValues["SOKU_VAL_MAX"];
                strcpy(pRIREKI_DATA[i1].SOKU_VAL_MAX		,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].SOKU_VAL_MIN,"0.0");
            if(VarIsNull(Query3->FieldValues["SOKU_VAL_MIN"])!=true)
            {
            	sBuf = Query3->FieldValues["SOKU_VAL_MIN"];
                strcpy(pRIREKI_DATA[i1].SOKU_VAL_MIN		,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].SOKU_OK,"0");
            if(VarIsNull(Query3->FieldValues["SOKU_OK"])!=true)
            {
            	sBuf = Query3->FieldValues["SOKU_OK"];
                strcpy(pRIREKI_DATA[i1].SOKU_OK		,sBuf.c_str());
            }
//DISP Result Read ///////////////////////////////////////////////////////////////////////
			strcpy(pRIREKI_DATA[i1].DISP_VAL01,"");
            if(VarIsNull(Query3->FieldValues["DISP_VAL01"])!=true)
            {
            	sBuf = Query3->FieldValues["DISP_VAL01"];
                strcpy(pRIREKI_DATA[i1].DISP_VAL01		,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].DISP_VAL02,"");
            if(VarIsNull(Query3->FieldValues["DISP_VAL02"])!=true)
            {
            	sBuf = Query3->FieldValues["DISP_VAL02"];
                strcpy(pRIREKI_DATA[i1].DISP_VAL02		,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].DISP_VAL03,"");
            if(VarIsNull(Query3->FieldValues["DISP_VAL03"])!=true)
            {
            	sBuf = Query3->FieldValues["DISP_VAL03"];
                strcpy(pRIREKI_DATA[i1].DISP_VAL03		,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].DISP_VAL04,"");
            if(VarIsNull(Query3->FieldValues["DISP_VAL04"])!=true)
            {
            	sBuf = Query3->FieldValues["DISP_VAL04"];
                strcpy(pRIREKI_DATA[i1].DISP_VAL04		,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].DISP_VAL05,"");
            if(VarIsNull(Query3->FieldValues["DISP_VAL05"])!=true)
            {
            	sBuf = Query3->FieldValues["DISP_VAL05"];
                strcpy(pRIREKI_DATA[i1].DISP_VAL05		,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].DISP_VAL06,"");
            if(VarIsNull(Query3->FieldValues["DISP_VAL06"])!=true)
            {
            	sBuf = Query3->FieldValues["DISP_VAL06"];
                strcpy(pRIREKI_DATA[i1].DISP_VAL06		,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].DISP_VAL07,"");
            if(VarIsNull(Query3->FieldValues["DISP_VAL07"])!=true)
            {
            	sBuf = Query3->FieldValues["DISP_VAL07"];
                strcpy(pRIREKI_DATA[i1].DISP_VAL07		,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].DISP_VAL08,"");
            if(VarIsNull(Query3->FieldValues["DISP_VAL08"])!=true)
            {
            	sBuf = Query3->FieldValues["DISP_VAL08"];
                strcpy(pRIREKI_DATA[i1].DISP_VAL08		,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].DISP_VAL09,"");
            if(VarIsNull(Query3->FieldValues["DISP_VAL09"])!=true)
            {
            	sBuf = Query3->FieldValues["DISP_VAL09"];
                strcpy(pRIREKI_DATA[i1].DISP_VAL09		,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].DISP_VAL10,"");
            if(VarIsNull(Query3->FieldValues["DISP_VAL10"])!=true)
            {
            	sBuf = Query3->FieldValues["DISP_VAL10"];
                strcpy(pRIREKI_DATA[i1].DISP_VAL10		,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].DISP_VAL_SU,"1");
            if(VarIsNull(Query3->FieldValues["DISP_VAL_SU"])!=true)
            {
            	sBuf = Query3->FieldValues["DISP_VAL_SU"];
                strcpy(pRIREKI_DATA[i1].DISP_VAL_SU		,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].DISP_VAL_AVE,"0.0");
            if(VarIsNull(Query3->FieldValues["DISP_VAL_AVE"])!=true)
            {
            	sBuf = Query3->FieldValues["DISP_VAL_AVE"];
                strcpy(pRIREKI_DATA[i1].DISP_VAL_AVE		,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].DISP_VAL_MAX,"0.0");
            if(VarIsNull(Query3->FieldValues["DISP_VAL_MAX"])!=true)
            {
            	sBuf = Query3->FieldValues["DISP_VAL_MAX"];
                strcpy(pRIREKI_DATA[i1].DISP_VAL_MAX		,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].DISP_VAL_MIN,"0.0");
            if(VarIsNull(Query3->FieldValues["DISP_VAL_MIN"])!=true)
            {
            	sBuf = Query3->FieldValues["DISP_VAL_MIN"];
                strcpy(pRIREKI_DATA[i1].DISP_VAL_MIN		,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].DISP_OK,"0");
            if(VarIsNull(Query3->FieldValues["DISP_OK"])!=true)
            {
            	sBuf = Query3->FieldValues["DISP_OK"];
                strcpy(pRIREKI_DATA[i1].DISP_OK		,sBuf.c_str());
            }
//日付、時刻、更新者の読み取り
            strcpy(pRIREKI_DATA[i1].SKTYMD,"0");
            if(VarIsNull(Query3->FieldValues["SKTYMD"])!=true)
            {
            	sBuf = Query3->FieldValues["SKTYMD"];
                strcpy(pRIREKI_DATA[i1].SKTYMD		,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].ADDYMD,"0");
            if(VarIsNull(Query3->FieldValues["ADDYMD"])!=true)
            {
            	sBuf = Query3->FieldValues["ADDYMD"];
                strcpy(pRIREKI_DATA[i1].ADDYMD		,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].ADDTIM,"0");
            if(VarIsNull(Query3->FieldValues["ADDTIM"])!=true)
            {
            	sBuf = Query3->FieldValues["ADDTIM"];
                strcpy(pRIREKI_DATA[i1].ADDTIM		,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].UPDYMD,"0");
            if(VarIsNull(Query3->FieldValues["UPDYMD"])!=true)
            {
            	sBuf = Query3->FieldValues["UPDYMD"];
                strcpy(pRIREKI_DATA[i1].UPDYMD		,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].UPDTIM,"0");
            if(VarIsNull(Query3->FieldValues["UPDTIM"])!=true)
            {
            	sBuf = Query3->FieldValues["UPDTIM"];
                strcpy(pRIREKI_DATA[i1].UPDTIM		,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].UPDCHR,"0");
            if(VarIsNull(Query3->FieldValues["UPDCHR"])!=true)
            {
            	sBuf = Query3->FieldValues["UPDCHR"];
                strcpy(pRIREKI_DATA[i1].UPDCHR		,sBuf.c_str());
            }
            strcpy(pRIREKI_DATA[i1].UPDCNT,"0");
            if(VarIsNull(Query3->FieldValues["UPDCNT"])!=true)
            {
            	sBuf = Query3->FieldValues["UPDCNT"];
                strcpy(pRIREKI_DATA[i1].UPDCNT		,sBuf.c_str());
            }
            ...
            //pRIREKI_DATA[i1].UPDNAME = "";
			//2003.05.17 E.Takase Add コメント↑
            if(VarIsNull(Query3->FieldValues["UPDNAME"])!=true)
            {
            	sBuf = Query3->FieldValues["UPDNAME"];
				//2003.05.17 E.Takase Add↓
                strcpy(pRIREKI_DATA[i1].UPDNAME , sBuf.c_str());
				//2003.05.17 E.Takase Add↑
				//2003.05.17 E.Takase Add コメント↓
...
//測定機器NO、測定項目NOの一致している場所を探索
        ...
        for( i1 = 0; i1 < i4;i1++)
        {
            ...
            while( 1 )
            {
                ...
                && (sBuf == sBuf1)
				&& (AnsiString(pSOZAI_OR_DATA[i2].HINBAN_SK) == AnsiString(pRIREKI_DATA[i1].HINBAN_SK))) break;
...
/////
			bSET_SOZAI = true;
			if(bLOTCHANGE == true)			//ロット割り当て画面で「変更」が押された場合の処理です
			{
	    		for(iLoop = 0;iLoop < 6; iLoop++)
				{
                	if(bSET_SOZAI == false ) break;
        	    	switch( iLoop )
					{
					case 0:
				...
				//ロット割り当て画面で「変更」が押下されているが、この項目は変更されていないのでSETフラグを落とす
					if(( AnsiString(pRIREKI_DATA[i1].HINBAN_SK) 	== sDTKSHIN_SZ)
					&&( iLotChange == 0))
					{
						bSET_SOZAI = false;
						break;
...
//履歴データのデータ読み取り（10件まで　11件以上を考えること）
			for(i3 = 0; i3 <  AnsiString(pRIREKI_DATA[i1].SOKU_SU).ToIntDef(0) ; i3++ )
            {
				if( bSET_SOZAI == true )
				{
					...
					//2003.05.17 E.Takase Add↑
					//2003.05.17 E.Takase Add コメント↓
      	            //pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow].UPDNAME = pRIREKI_DATA[i1].UPDNAME;
					//2003.05.17 E.Takase Add コメント↑
					//2003.05.10 E.Takase Add↑
					//2003.05.10 E.Takase Add コメント↓
      	            //pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow].UPDNAME = GetTANTO(AnsiString(pRIREKI_DATA[i1].UPDCHR));
					//2003.05.10 E.Takase Add コメント↑
				}
                switch( i3 )
                {
                case 0:
                	if( bSET_SOZAI == true || bSET_SOZAI == false )
                    {
//                    	SGr_vSOKU_SOZAI->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL01;
                    ...
                    {
//                    	SGr_vSOKU_SOZAI->Cells[iCol][iRow] = pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow].RESULT;
                    }
                    break;
                case 1:
                	if( bSET_SOZAI == true || bSET_SOZAI == false)
                    {
//                    	SGr_vSOKU_SOZAI->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL02;
...
//                    	SGr_vSOKU_SOZAI->Cells[iCol][iRow] = pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow].RESULT;
                ...
                case 2:
                	if( bSET_SOZAI == true || bSET_SOZAI == false)
                    {
//                    	SGr_vSOKU_SOZAI->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL03;
...
//                    	SGr_vSOKU_SOZAI->Cells[iCol][iRow] = pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow].RESULT;
                ...
                case 3:
                	if( bSET_SOZAI == true || bSET_SOZAI == false)
                    {
//                    	SGr_vSOKU_SOZAI->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL04;
...
//                    	SGr_vSOKU_SOZAI->Cells[iCol][iRow] = pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow].RESULT;
                ...
                case 4:
                	if( bSET_SOZAI == true || bSET_SOZAI == false )
                    {
//                    	SGr_vSOKU_SOZAI->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL05;
...
//                    	SGr_vSOKU_SOZAI->Cells[iCol][iRow] = pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow].RESULT;
                ...
                case 5:
                	if( bSET_SOZAI == true || bSET_SOZAI == false )
                    {
//                    	SGr_vSOKU_SOZAI->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL06;
...
//                    	SGr_vSOKU_SOZAI->Cells[iCol][iRow] = pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow].RESULT;
                ...
                case 6:
                	if( bSET_SOZAI == true || bSET_SOZAI == false )
                    {
//                    	SGr_vSOKU_SOZAI->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL07;
...
//                    	SGr_vSOKU_SOZAI->Cells[iCol][iRow] = pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow].RESULT;
                ...
                case 7:
                	if( bSET_SOZAI == true || bSET_SOZAI == false )
                    {
//                    	SGr_vSOKU_SOZAI->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL08;
...
//                    	SGr_vSOKU_SOZAI->Cells[iCol][iRow] = pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow].RESULT;
                ...
                case 8:
                	if( bSET_SOZAI == true || bSET_SOZAI == false )
                    {
//                    	SGr_vSOKU_SOZAI->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL09;
...
//                    	SGr_vSOKU_SOZAI->Cells[iCol][iRow] = pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow].RESULT;
                ...
                case 9:
                	if( bSET_SOZAI == true || bSET_SOZAI == false )
                    {
//                    	SGr_vSOKU_SOZAI->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL10;
...
//                    	SGr_vSOKU_SOZAI->Cells[iCol][iRow] = pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow].RESULT;
                ...
                default:
                    ...
                    // Initial Query
                    Form1->Query8->Close();
                    Form1->Query8->SQL->Clear();
                    Form1->Query8->SQL->Add(sBuf);
                    Form1->Query8->Open();
                    Form1->Query8->Active = true;
                    while( Query8->Eof != true )
                    {
					    sBuf = "";
					    if ( VarIsNull(Query8->FieldValues["SOKU_NO"]) != true )
					    {
                    	    sBuf = Query8->FieldValues["SOKU_NO"];
    					}
                        if(AnsiString(pRIREKI_DATA[i1].SOKU_SU).ToIntDef(0) < sBuf.ToIntDef(0)) break;
                        i3 = sBuf.ToIntDef(11)-11;
					    if ( VarIsNull(Query8->FieldValues["SOKU_VAL"]) != true )
					    {
                    	    sBuf = Query8->FieldValues["SOKU_VAL"];
    					}
		    			...
		    			sBuf = "";
			    		if ( VarIsNull(Query8->FieldValues["DISP_VAL"]) != true )
				    	{
                        	sBuf = Query8->FieldValues["DISP_VAL"];
    					}
                        //strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow].DISP_RESULT	, sBuf.c_str());
                        strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3].DISP_RESULT	, sBuf.c_str());
                    //if( bSET_SOZAI == true)
                    //{
//                    	SGr_vSOKU_SOZAI->Cells[iCol][iRow] = sBuf;
                    //    strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow].RESULT		, sBuf.c_str());
                    //    sBuf = Query8->FieldValues["DISP_VAL"];
                    //    strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow].DISP_RESULT	, sBuf.c_str());
                    //}
...
//                  //  	SGr_vSOKU_SOZAI->Cells[iCol][iRow] = pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow].RESULT;
                    //}
    	            	strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].DTKSHIN 		, pRIREKI_DATA[i1].DTKSHIN);
        	            strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].LOTNO 		, pRIREKI_DATA[i1].LOTNO);
		    		   	strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].UNQ_NO 		, pRIREKI_DATA[i1].UNQ_NO);
                	    strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].ORDER_NO		, pRIREKI_DATA[i1].ORDER_NO);
			  	    	strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].SKB_NO 		, pRIREKI_DATA[i1].SKB_NO);
				   	    strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].HINBAN_SK	, pRIREKI_DATA[i1].HINBAN_SK);
    			   	  	strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].KOMK_NO 		, pRIREKI_DATA[i1].KOMK_NO);
            	        strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].KOMK_NM 		, pRIREKI_DATA[i1].KOMK_NM);
		    		   	strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].KOMK_ENG		, pRIREKI_DATA[i1].KOMK_ENG);
			      		strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].KOMK_SUB		, pRIREKI_DATA[i1].KOMK_SUB);
	                    strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].KOMK_SUB_ENG , pRIREKI_DATA[i1].KOMK_SUB_ENG);
    	                strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].KIKI_NO 		, pRIREKI_DATA[i1].KIKI_NO);
    			 	   	strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].KIKI_NM 		, pRIREKI_DATA[i1].KIKI_NM);
	    			  	strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].TANI    		, pRIREKI_DATA[i1].TANI);
		    		   	strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].KIKA_TYPE	, pRIREKI_DATA[i1].KIKA_TYPE);
			    	 	strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].KIKA_KIND	, pRIREKI_DATA[i1].KIKA_KIND);
			   	    	strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].KIKA_DECI	, pRIREKI_DATA[i1].KIKA_DECI);
            	        strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].KIKA_NUM1	, pRIREKI_DATA[i1].KIKA_NUM1);
                    	strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].KIKA_NUM2	, pRIREKI_DATA[i1].KIKA_NUM2);
	                    strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].KIKA_NUM3	, pRIREKI_DATA[i1].KIKA_NUM3);
    	                strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].KIKA_STR		, pRIREKI_DATA[i1].KIKA_STR);
        	            strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].KIKA_SUB1	, pRIREKI_DATA[i1].KIKA_SUB1);
            	        strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].KIKA_SUB2	, pRIREKI_DATA[i1].KIKA_SUB2);
	                    strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].KIKA_SUB3	, pRIREKI_DATA[i1].KIKA_SUB3);
        	            strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].SOKU_SU 		, pRIREKI_DATA[i1].SOKU_SU);
            	        strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].SOKU_FLG		, pRIREKI_DATA[i1].SOKU_FLG);
                	    strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].TOKU_FLG		, pRIREKI_DATA[i1].TOKU_FLG);
                    	strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].KIKA_PRT		, pRIREKI_DATA[i1].KIKA_PRT);
	                    strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].VALUE_TYPE	, pRIREKI_DATA[i1].VALUE_TYPE);
    	                strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].VALUE_DECI	, pRIREKI_DATA[i1].VALUE_DECI);
            	        strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].SOKU_VAL_SU	, pRIREKI_DATA[i1].SOKU_VAL_SU);
                	    strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].SOKU_VAL_AVE	, pRIREKI_DATA[i1].SOKU_VAL_AVE);
	                    strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].SOKU_VAL_MAX	, pRIREKI_DATA[i1].SOKU_VAL_MAX);
    	                strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].SOKU_VAL_MIN	, pRIREKI_DATA[i1].SOKU_VAL_MIN);
        	            strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].SOKU_OK		, pRIREKI_DATA[i1].SOKU_OK);
            	        strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].DISP_VAL_SU	, pRIREKI_DATA[i1].DISP_VAL_SU);
    	                strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].DISP_VAL_AVE	, pRIREKI_DATA[i1].DISP_VAL_AVE);
        	            strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].DISP_VAL_MAX	, pRIREKI_DATA[i1].DISP_VAL_MAX);
            	        strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].DISP_VAL_MIN	, pRIREKI_DATA[i1].DISP_VAL_MIN);
                	    strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].DISP_OK		, pRIREKI_DATA[i1].DISP_OK);
	                    strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].SKTYMD		, pRIREKI_DATA[i1].SKTYMD);
    	                strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].ADDYMD		, pRIREKI_DATA[i1].ADDYMD);
            	        strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].ADDTIM		, pRIREKI_DATA[i1].ADDTIM);
                	    strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].UPDYMD		, pRIREKI_DATA[i1].UPDYMD);
	                    strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].UPDTIM		, pRIREKI_DATA[i1].UPDTIM);
    	                strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].UPDCHR		, pRIREKI_DATA[i1].UPDCHR);
        	            strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].UPDCNT		, pRIREKI_DATA[i1].UPDCNT);
					    //2003.05.17 E.Takase Add↓
          	            strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow + i3 ].UPDNAME , pRIREKI_DATA[i1].UPDNAME);
	    				//2003.05.17 E.Takase Add↑
                        Query8->Next();
                    }
                    i3 =  AnsiString(pRIREKI_DATA[i1].SOKU_SU).ToIntDef(0) + 1;
...
//履歴データのデータ読み取り（10件まで　11件以上を考えること）
            ...
            for(i3 = 0; i3 <  AnsiString(pRIREKI_DATA[i1].SOKU_SU).ToIntDef(0) ; i3++ )
            {
                ...
                {
                case 0:
//                	SGr_vRIRE_SOZAI->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL01;
					strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow].RESULT		, pRIREKI_DATA[i1].SOKU_VAL01);
					strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow].DISP_RESULT	, pRIREKI_DATA[i1].DISP_VAL01);
                    break;
                case 1:
//                	SGr_vRIRE_SOZAI->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL02;
					strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow].RESULT		, pRIREKI_DATA[i1].SOKU_VAL02);
					strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow].DISP_RESULT	, pRIREKI_DATA[i1].DISP_VAL02);
                    break;
                case 2:
//                	SGr_vRIRE_SOZAI->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL03;
					strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow].RESULT		, pRIREKI_DATA[i1].SOKU_VAL03);
					strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow].DISP_RESULT	, pRIREKI_DATA[i1].DISP_VAL03);
                    break;
                case 3:
//                	SGr_vRIRE_SOZAI->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL04;
					strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow].RESULT		, pRIREKI_DATA[i1].SOKU_VAL04);
					strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow].DISP_RESULT	, pRIREKI_DATA[i1].DISP_VAL04);
                    break;
                case 4:
//                	SGr_vRIRE_SOZAI->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL05;
					strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow].RESULT		, pRIREKI_DATA[i1].SOKU_VAL05);
					strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow].DISP_RESULT	, pRIREKI_DATA[i1].DISP_VAL05);
                    break;
                case 5:
//                	SGr_vRIRE_SOZAI->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL06;
					strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow].RESULT		, pRIREKI_DATA[i1].SOKU_VAL06);
					strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow].DISP_RESULT	, pRIREKI_DATA[i1].DISP_VAL06);
                    break;
                case 6:
//                	SGr_vRIRE_SOZAI->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL07;
					strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow].RESULT		, pRIREKI_DATA[i1].SOKU_VAL07);
					strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow].DISP_RESULT	, pRIREKI_DATA[i1].DISP_VAL07);
                    break;
                case 7:
//                	SGr_vRIRE_SOZAI->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL08;
					strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow].RESULT		, pRIREKI_DATA[i1].SOKU_VAL08);
					strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow].DISP_RESULT	, pRIREKI_DATA[i1].DISP_VAL08);
                    break;
                case 8:
//                	SGr_vRIRE_SOZAI->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL09;
					strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow].RESULT		, pRIREKI_DATA[i1].SOKU_VAL09);
					strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow].DISP_RESULT	, pRIREKI_DATA[i1].DISP_VAL09);
                    break;
                case 9:
//                	SGr_vRIRE_SOZAI->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL10;
					strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow].RESULT		, pRIREKI_DATA[i1].SOKU_VAL10);
					strcpy(pCELLINFO_SOKU_SOZAI[(iCol * SGr_vSOKU_SOZAI->RowCount) + iRow].DISP_RESULT	, pRIREKI_DATA[i1].DISP_VAL10);
                    break;
                default:
...
//////////////////////////////////////////////////////////////
	while( 1 )
	{
    	//if(i1 >= SGr_vRIRE_SOZAI->RowCount ) break;
		//iUNQNO = AnsiString(pCELLINFO_RIRE_SOZAI[i1].UNQ_NO).ToIntDef(0);
		//sUNQNO += " " + AnsiString(pCELLINFO_RIRE_SOZAI[i1].UNQ_NO) + " ,";
		//while(iUNQNO == AnsiString(pCELLINFO_RIRE_SOZAI[i1].UNQ_NO).ToIntDef(0))
		//{
		//	i1++;
		//	if(i1 >= SGr_vRIRE_SOZAI->RowCount ) break;
		//}
    	//if(i1 >= SGr_vRIRE_SOZAI->RowCount ) break;
		//iUNQNO = AnsiString(pCELLINFO_RIRE_SOZAI[i1].KOMK_NO).ToIntDef(0);
		//sUNQNO += " " + AnsiString(pCELLINFO_RIRE_SOZAI[i1].KOMK_NO) + " ,";
		//while(iUNQNO == AnsiString(pCELLINFO_RIRE_SOZAI[i1].KOMK_NO).ToIntDef(0))
		//{
		//	i1++;
		//	if(i1 >= SGr_vRIRE_SOZAI->RowCount ) break;
		//}
...
//出荷検査のグリッドをすべて灰色にする
        ...
        {
//        	SGr_vRIRE_SOZAI->Font->Color = clGray;
//            SGr_vRIRE_SOZAI->Cells[iCol][i2]="不要";
        }
//測定機器NO、測定項目NOの一致している場所を探索
		i4 = i1;
        iRow = 1;
...
//                	SGr_vRIRE_SOZAI->Cells[iCol][iRow] = pRIREKI_DATA[i1].SOKU_VAL10;
                ...
                default:
    				...
    				sBuf += " AND LOTNO = '"	 + AnsiString(pCELLINFO_RIRE_SOZAI[(iCol * SGr_vRIRE_SOZAI->RowCount) + iRow].LOTNO) 	+ "'";                        //出荷検査項目、材料証明項目
    				sBuf += " AND UNQ_NO = "	 + AnsiString(pCELLINFO_RIRE_SOZAI[(iCol * SGr_vRIRE_SOZAI->RowCount) + iRow].UNQ_NO) 	+ " ";                        //出荷検査項目、材料証明項目
    				//sBuf += " AND SOKU_NO = "	 + AnsiString(i3+1) 	+ " ";                        //出荷検査項目、材料証明項目
                    sBuf += " ORDER BY SOKU_NO ASC";
					//For Debug
                    // 2003.06.03 k.y
...
void __fastcall TForm1::GetKIKAKU(int iIndex,double *dMAX,double *dMIN)
{
    ...
    if( AnsiString(pORDATA[iIndex].KIKA_TYPE).ToIntDef(0) != 3 )
    {
        ...
        //2003.05.08 E.Takase Add コメント↓
    	//try{
    	//	*dMAX = AnsiString(pORDATA[iIndex].KIKA_NUM1).ToDouble();
        //	*dMIN = AnsiString(pORDATA[iIndex].KIKA_NUM1).ToDouble();
        //}
		//catch(EConvertError& e){
		//	*dMAX=0.0;
		//	*dMIN=0.0;
        ...
        case 1:
            ...
            case 1:
        		...
        		//2003.05.08 E.Takase Add コメント↓
				//try{
            	//	*dMAX = AnsiString(pORDATA[iIndex].KIKA_NUM2).ToDouble();
                //	*dMIN = AnsiString(pORDATA[iIndex].KIKA_NUM1).ToDouble();
				//}
				//catch(EConvertError& e){
				//	*dMAX=0.0;
            ...
            case 3:
				...
				//try{
                //	*dMIN = AnsiString(pORDATA[iIndex].KIKA_NUM1).ToDouble();
            	//	*dMAX = AnsiString(pORDATA[iIndex].KIKA_NUM1).ToDouble()*1.1;
				//}
				//catch(EConvertError& e){
				//	*dMAX=0.0;
				//	*dMIN=0.0;
        ...
        case 2:
            ...
            case 1:
				...
				//try{
            	//	*dMAX += AnsiString(pORDATA[iIndex].KIKA_NUM2).ToDouble();
                //	*dMIN += AnsiString(pORDATA[iIndex].KIKA_NUM3).ToDouble();
                //}
				//catch(EConvertError& e){
				//	*dMAX=0.0;
				//	*dMIN=0.0;
...
/* k.y 2003.05.17
    // メモリの開放
    if( hSPACE_DATA ){
        GlobalUnlock( hSPACE_DATA );
        hSYUKA_DATA = GlobalFree( hSPACE_DATA );
    }

    // メモリの確保
    hSPACE_DATA = GlobalAlloc( GHND, sizeof(struct KSM02_DATA) * (100) );
    if( hSPACE_DATA == NULL ){
        SBr1->Panels->Items[0]->Text = "メモリ領域を確保できません。(01)";
        SBr1->Update();
        Beep();
    ...
    pSPACE_DATA = (struct KSM02_DATA *)GlobalLock(hSPACE_DATA);
    if( hSPACE_DATA == NULL ){
        SBr1->Panels->Items[0]->Text = "メモリ領域を確保できません。(02)";
        SBr1->Update();
        Beep();
...
//-----------------------------------------------------------
		    ...
		    //Form1->SGr_vRIRE_SOZAI->ColCount = 0;
		    //Form1->SGr_vSOKU_SOZAI->ColCount = 0;
		    //Form1->SGr_vRIRE_SYUKA->ColCount = 0;
		    //Form1->SGr_vSOKU_SYUKA->ColCount = 0;
            Form1->SGr_tRIRE->ColCount = 0;
            // 2003/05/16 A.Tamura
            //Form1->Pnl_DUMMY3->Visible = false;
            // 2003/05/16 A.Tamura
            //Form1->Pnl_DUMMY4->Visible = false;
