...
// Initial Query
    ...
    sBuf += " DTKSHIN, LOTNO, BNKT_UNQ_NO, UNQ_NO,";
...
//例外データの書き込み
    for(i2 = 10 ; i2 < AnsiString(pBUN_DATA[iCol*99+i1].SOKU_SU).ToIntDef(0) ; i2++)
    {
        ...
        sBuf += " DTKSHIN, LOTNO, BNKT_UNQ_NO, SOKU_NO, SOKU_VAL )";
...
//    ②KSD11（測定実績分割データ）、KSD12（測定実績分割例外データ）読み込み表示
//  パラメタ説明
//    なし。
//
//  戻り値
...
//分割表で測定されたロットの読み込み
	...
	for(i2 = 0;i2 < iCBO_INDEX;i2++)
	{
        ...
        // 2003/10/8 A.Tamura ''の場合' '(半角の空白1文字)入っている可能性があるので検索条件
        //                    から外し後で、Trimして比較します
		//sBuf += " AND KOMK_SUB = '" + AnsiString(pCBO_SELECT[i2].KOMK_SUB) + "'";

		//sBuf += " AND UNQ_NO = " + AnsiString(pCBO_SELECT[i2].UNQ_NO);
		...
		//2003.05.12 E.Takase Add↑
		//2003.05.12 E.Takase Add コメント↓
		//sBuf += " ORDER BY LOTNO,UPDYMD,UPDTIM";
		//2003.05.12 E.Takase Add コメント↑
...
//  　②機能的には値を入力するセルですが、規格の判定や一番下のグリッドの入力で保存処理が出来るように作成してあります。
//
//  パラメタ説明
//    TObject *Sender	：呼び出し元のVCLオブジェクト
//  　int ACol          ：
