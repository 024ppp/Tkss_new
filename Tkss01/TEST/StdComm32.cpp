...
CommOpen(LPCTSTR pszComPort, LPCTSTR pszBaudRate, LPCTSTR pszConfig)
{
	...
	dcb.ByteSize = (BYTE)nByteSize;
	dcb.Parity = (BYTE)nParity;
	dcb.StopBits = (BYTE)nStopBits;
	...
	to.ReadIntervalTimeout = 500;		// 次のキャラクタを受信するまでのタイムアウト
	to.ReadTotalTimeoutMultiplier = 20;	// Read : 1バイトに対するタイムアウト
	to.ReadTotalTimeoutConstant = 500;	// Read : 0バイト時のタイムアウト
	to.WriteTotalTimeoutMultiplier = 20;// Write : 1バイトに対するタイムアウト
