...
void __fastcall TForm1::FormCreate(TObject *Sender)
{
	...
	AnsiString sDATE, sYY, sMM, sDD;                                  //onishi 生管検索の日付初期化
    int iYY;
...
void __fastcall TForm1::BtnF06Click(TObject *Sender)
{
    ...
    if( !BtnF06->Tag ){
		...
		case 1:
        	...
        	Form3->sNOW_HINBAN	= SGr_tSOKU->Cells[SGr_vSOKU_SYUKA->Col][1];
			break;
		case 2:
        	Form3->sNOW_LOTNO	= SGr_tRIRE->Cells[SGr_vRIRE_SYUKA->Col][0];
        	Form3->sNOW_HINBAN	= SGr_tRIRE->Cells[SGr_vRIRE_SYUKA->Col][1];
			break;
		case 3:
        	Form3->sNOW_LOTNO	= SGr_tSOKU->Cells[SGr_vSOKU_SOZAI->Col][0];
        	Form3->sNOW_HINBAN	= SGr_tSOKU->Cells[SGr_vSOKU_SOZAI->Col][1];
			break;
		case 4:
        	Form3->sNOW_LOTNO	= SGr_tRIRE->Cells[SGr_vRIRE_SOZAI->Col][0];
        	Form3->sNOW_HINBAN	= SGr_tRIRE->Cells[SGr_vRIRE_SOZAI->Col][1];
			break;
		}
...
void __fastcall TForm1::BtnF07Click(TObject *Sender)
{
    ...
    if( !BtnF07->Tag ){
        ...
        sNowSEIKAN_YMD = Edt_SEIKAN_YMD->Text;                 //onishi
        ...
        else
        {
            ...
            if(AnsiString(pKSM01_DATA.SOKCHR) != "0"
            && AnsiString(pKSM01_DATA.SOKCHR) != AnsiString(Form1->USER_COD).c_str())
            {
	            bNowF7 = false;
	            MessageDlg("他のユーザー： " + AnsiString(pKSM01_DATA.SOKCHR) + " が測定中です。"
...
void __fastcall TForm1::BtnF11Click(TObject *Sender)
{
    ...
    if( !BtnF11->Tag )
    {
        ...
        //20170509 Y.Onishi 保存後メッセージ追加
        sBuf  = "保存しました。";
        MsgButton2 << mbOK;
        MessageDlg( sBuf, mtInformation, MsgButton2, 0);
        return;
    }
}
...
//
//---------------------------------------------------------------------------
void __fastcall TForm1::KSM01_LOCKON(void)
{
    AnsiString sUpdBuf;
    ...
    sUpdBuf = " UPDATE KSM01 SET SOKCHR =  " + AnsiString(Form1->USER_COD);
    sUpdBuf = sUpdBuf + " WHERE DTKSHIN = '" + sNowDTKSHIN + "'";
    sUpdBuf = sUpdBuf + " AND   REVCNT  = 0";

    Query1->SQL->Add(sUpdBuf);
...
//
//---------------------------------------------------------------------------
void __fastcall TForm1::KSM01_LOCKOFF(void)
{
    AnsiString sUpdBuf;
    ...
    sUpdBuf = " UPDATE KSM01 SET SOKCHR = 0 ";
    sUpdBuf = sUpdBuf + " WHERE SOKCHR  = " + AnsiString(Form1->USER_COD);
