...
private:	// ユーザー宣言
	...
	struct KSD01_DATA {
		...
		char	DTKSHIN5_SZ[21];	// 素材用得意先品番5
		char	DTKSHIN6_SZ[21];	// 素材用得意先品番6
		char	HINBAN1_SZ[21];	    // 素材用自社品番1
		char	HINBAN2_SZ[21];	    // 素材用自社品番2
		char	HINBAN3_SZ[21];	    // 素材用自社品番3
		char	HINBAN4_SZ[21];	    // 素材用自社品番4
		char	HINBAN5_SZ[21];	    // 素材用自社品番5
		char	HINBAN6_SZ[21];	    // 素材用自社品番6
		char	LOT_LINK_SZ[2];		// 素材用ロットのリンク
		char	SOKUTEI_NUM[3];		// 測定項目数
...
public:		// ユーザー宣言
    ...
    AnsiString	sNOW_HINBAN;		// (Input)項目編集対象の得意先品番
